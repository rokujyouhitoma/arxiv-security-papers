...
e++;else if(/^(\-{3,}|\*{3,})$/.test(h))f.push({type:"HR"}),e++;else if(h.startsWith("|")&&h.endsWith("|")){for(d=[];e<a.length&&a[e].trim().startsWith("|")&&a[e].trim().endsWith("|");)d.push(a[e].trim()),e++;if(2<=d.length){const k=u=>u.split("|").slice(1,-1).map(p=>p.trim());h=k(d[0]);let m=1;d[1].includes("---")&&(m=2);d=d.slice(m).map(k);f.push({type:"TABLE",headers:h,rows:d})}}else if(/^[\-\*]\s+/.test(h)){for(h=[];e<a.length&&/^[\-\*]\s+/.test(a[e].trim());)h.push(a[e].trim().replace(/^[\-\*]\s+/,
...
const q=await l.json(),n=performance.now(),r=q.profile;J.textContent=r&&void 0!==r.total_ms?`\u26a1 ${r.total_ms} ms (${r.candidates_evaluated}\u4ef6\u8a55\u4fa1 / \u5168${r.total_documents}\u4ef6)`:`${Math.round(n-c)} ms \u3067\u53d6\u5f97`;"success"===q.status&&q.results?f(q.results):(x.innerHTML='<p class="loading-text">\u8a72\u5f53\u3059\u308b\u8ad6\u6587\u306f\u898b\u3064\u304b\u308a\u307e\u305b\u3093\u3067\u3057\u305f\u3002</p>',D.textContent="\u691c\u7d22\u7d50\u679c (0\u4ef6)")}catch(g){x.innerHTML=
...
c.highlight?`<div class="card-snippet">${c.highlight}</div>`:`<p class="card-desc">${k(c.description||"\u8981\u7d04\u60c5\u5831\u306a\u3057")}</p>`;return`
      <div class="glass-panel paper-card" onclick="openPaperModal('${k(c.id)}')">
        ...
        <div class="card-footer">
          <div class="card-tags">
            ${(c.tags||[]).slice(0,3).map(q=>`<span class="mini-tag">${k(q)}</span>`).join("")}
          </div>
      ...
      `}).join("")}async function e(b,c){try{const g=await (await fetch(`/api/paper/${encodeURIComponent(b)}/related`)).json();if("success"===g.status&&g.related_papers&&0<g.related_papers.length){const l=document.createElement("div");l.className="related-papers-section";b="";g.mermaid_graph&&(b=`
          ...
          `);const q=g.related_papers.map(n=>`
          <div class="related-card" onclick="openPaperModal('${k(n.target_id||"")}')">
            ...
            <div class="card-tags">
              ${(n.shared_keywords||[]).slice(0,2).map(r=>`<span class="mini-tag">${k(r)}</span>`).join("")}
            </div>
          </div>
        `).join("");l.innerHTML=`
...
y.innerHTML=g.html;window.MarkdownCompiler.renderMermaid(y)}}catch(c){y.innerHTML=`<p style="color:#ef4444;">\u30c8\u30ec\u30f3\u30c9\u53d6\u5f97\u30a8\u30e9\u30fc: ${k(c.message)}</p>`}}function k(b){return b?b.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}let m="",u="monthly";const p=document.getElementById("searchInput"),K=document.getElementById("searchBtn"),x=document.getElementById("resultsGrid"),D=document.getElementById("resultsCount"),J=document.getElementById("searchTime"),
...
try{const c=await (await fetch(`/api/paper/${encodeURIComponent(b)}`)).json();if("success"===c.status&&c.content){E.textContent=c.content.match(/title:\s*"(.*?)"/)?.[1]||b;F.textContent=c.content.match(/title_ja:\s*"(.*?)"/)?.[1]||"";const g=window.MarkdownCompiler.compile(c.content);v.innerHTML=g.html;e(b,v);window.MarkdownCompiler.renderMermaid(v)}}catch(c){v.innerHTML=`<p style="color:#ef4444;">\u53d6\u5f97\u30a8\u30e9\u30fc: ${k(c.message)}</p>`}};M.addEventListener("click",d);window.addEventListener("keydown",

...
e++;else if(/^(\-{3,}|\*{3,})$/.test(h))f.push({type:"HR"}),e++;else if(h.startsWith("|")&&h.endsWith("|")){for(d=[];e<a.length&&a[e].trim().startsWith("|")&&a[e].trim().endsWith("|");)d.push(a[e].trim()),e++;if(2<=d.length){const k=u=>u.split("|").slice(1,-1).map(q=>q.trim());h=k(d[0]);let m=1;d[1].includes("---")&&(m=2);d=d.slice(m).map(k);f.push({type:"TABLE",headers:h,rows:d})}}else if(/^[\-\*]\s+/.test(h)){for(h=[];e<a.length&&/^[\-\*]\s+/.test(a[e].trim());)h.push(a[e].trim().replace(/^[\-\*]\s+/,
...
y.innerHTML=g.html;window.MarkdownCompiler.renderMermaid(y)}}catch(c){y.innerHTML=`<p style="color:#ef4444;">\u30c8\u30ec\u30f3\u30c9\u53d6\u5f97\u30a8\u30e9\u30fc: ${k(c.message)}</p>`}}function k(b){return b?b.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}let m="",u="monthly";const q=document.getElementById("searchInput"),L=document.getElementById("searchBtn"),x=document.getElementById("resultsGrid"),D=document.getElementById("resultsCount"),K=document.getElementById("searchTime"),
M=document.getElementById("totalPapersCount"),z=document.getElementById("paperModal"),N=document.getElementById("closeModalBtn"),O=document.getElementById("modalPaperId"),E=document.getElementById("modalPaperTitle"),F=document.getElementById("modalPaperTitleJa"),v=document.getElementById("modalPaperBody"),G=document.getElementById("modalArxivLink"),H=document.getElementById("modalPdfLink"),I=document.getElementById("modalOkfLink"),y=document.getElementById("trendContent"),A=document.getElementById("mcpToolSelect"),
w=document.getElementById("mcpArgsInput"),P=document.getElementById("runMcpBtn"),B=document.getElementById("mcpOutput");var t=new URLSearchParams(window.location.search);const J=t.get("q")||t.get("query"),C=t.get("tag")||t.get("category");C&&(m=C,document.querySelectorAll(".filter-chip").forEach(b=>{b.getAttribute("data-tag")===C?b.classList.add("active"):b.classList.remove("active")}));(async function(){try{const b=await (await fetch("/api/stats")).json();b.total_papers&&(M.textContent=Number(b.total_papers).toLocaleString())}catch(b){console.warn("Stats fetch failed",
b)}})();t=null!==J?J:"\u30da\u30f3\u30c6\u30b9\u30c8";q.value=t;a(t,!1);document.querySelectorAll(".nav-btn").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll(".nav-btn").forEach(g=>g.classList.remove("active"));document.querySelectorAll(".tab-content").forEach(g=>g.classList.remove("active"));b.classList.add("active");const c=b.getAttribute("data-tab");document.getElementById(c).classList.add("active");"trendsTab"===c&&h(u)})});document.querySelectorAll(".filter-chip").forEach(b=>
...
try{const c=await (await fetch(`/api/paper/${encodeURIComponent(b)}`)).json();if("success"===c.status&&c.content){I&&c.path&&(I.href="/"+encodeURI(c.path));E.textContent=c.content.match(/title:\s*"(.*?)"/)?.[1]||b;F.textContent=c.content.match(/title_ja:\s*"(.*?)"/)?.[1]||"";const g=window.MarkdownCompiler.compile(c.content);v.innerHTML=g.html;e(b,v);window.MarkdownCompiler.renderMermaid(v)}}catch(c){v.innerHTML=`<p style="color:#ef4444;">\u53d6\u5f97\u30a8\u30e9\u30fc: ${k(c.message)}</p>`}};N.addEventListener("click",
d);window.addEventListener("keydown",b=>{"Escape"!==b.key||z.classList.contains("hidden")||d()});document.querySelectorAll(".period-btn").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll(".period-btn").forEach(c=>c.classList.remove("active"));b.classList.add("active");u=b.getAttribute("data-period");h(u)})});A.addEventListener("change",()=>{const b=A.value;"search_security_papers"===b?w.value=JSON.stringify({query:"\u30da\u30f3\u30c6\u30b9\u30c8\u81ea\u52d5\u5316",top_k:5},null,
2):"get_paper_summary"===b?w.value=JSON.stringify({arxiv_id:"2608.12996"},null,2):"get_latest_trends"===b?w.value=JSON.stringify({period:"monthly"},null,2):"query_attack_technique"===b&&(w.value=JSON.stringify({technique_id:"T1059"},null,2))});P.addEventListener("click",async()=>{B.textContent="\u26a1 MCP JSON-RPC \u547c\u3073\u51fa\u3057\u4e2d...";try{const b=A.value,c=JSON.parse(w.value),g=await (await fetch("/api/mcp",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:b,
arguments:c})})).json();B.textContent=JSON.stringify(g,null,2)}catch(b){B.textContent=`\u30a8\u30e9\u30fc: ${b.message}`}})});

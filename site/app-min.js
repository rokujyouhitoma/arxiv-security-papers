...
a=a.replace(/`(.*?)`/g,'<code class="md-inline-code">$1</code>');return a=a.replace(/\[(.*?)\]\((.*?)\)/g,'<a href="$2" class="md-link" target="_blank" rel="noopener">$1</a>')}}"undefined"!==typeof module&&module.exports&&(module.exports={MarkdownEvaluator});class MarkdownRenderer{render(a){if(!a||"DOCUMENT"!==a.type)return"";const f=[],e=[];for(const h of a.children){var d=h.evaluated||{};switch(h.type){case "HEADING":f.push(`<h${d.level} class="md-h${d.level}">${d.content}</h${d.level}>`);break;case "PARAGRAPH":f.push(`<p class="md-p">${d.content}</p>`);break;case "HR":f.push('<hr class="md-hr" />');break;case "BLOCKQUOTE":f.push(`<blockquote class="md-blockquote">${d.content}</blockquote>`);break;case "LIST":a=d.items.map(k=>`<li>${k}</li>`).join("");
f.push(`<ul class="md-list">${a}</ul>`);break;case "TABLE":a=d.headers.map(k=>`<th>${k}</th>`).join("");d=d.rows.map(k=>`<tr>${k.map(m=>`<td>${m}</td>`).join("")}</tr>`).join("");f.push(`
...
const r=await l.json(),n=performance.now(),p=r.profile;K.textContent=p&&void 0!==p.total_ms?`\u26a1 ${p.total_ms} ms (${p.candidates_evaluated}\u4ef6\u8a55\u4fa1 / \u5168${p.total_documents}\u4ef6)`:`${Math.round(n-c)} ms \u3067\u53d6\u5f97`;"success"===r.status&&r.results?f(r.results):(x.innerHTML='<p class="loading-text">\u8a72\u5f53\u3059\u308b\u8ad6\u6587\u306f\u898b\u3064\u304b\u308a\u307e\u305b\u3093\u3067\u3057\u305f\u3002</p>',D.textContent="\u691c\u7d22\u7d50\u679c (0\u4ef6)")}catch(g){x.innerHTML=
...
c.highlight?`<div class="card-snippet">${c.highlight}</div>`:`<p class="card-desc">${k(c.description||"\u8981\u7d04\u60c5\u5831\u306a\u3057")}</p>`,r=c.path?"/"+encodeURI(c.path):"#",n=`/preview/${encodeURIComponent(c.id)}`;return`
      <div class="glass-panel paper-card">
        ...
        <div class="card-footer">
          <div class="card-tags">
            ${(c.tags||[]).slice(0,2).map(p=>`<span class="mini-tag">${k(p)}</span>`).join("")}
          </div>
          <div class="card-actions-row">
            <a href="${r}" target="_blank" rel="noopener" class="card-action-link" title="\u751f\u306e OKF Markdown \u3092\u30d7\u30ec\u30fc\u30f3\u30c6\u30ad\u30b9\u30c8\u3067\u8868\u793a" onclick="event.stopPropagation()">\ud83d\udcdd .md</a>
            <a href="${n}" target="_blank" rel="noopener" class="card-action-link" title="\u30b9\u30bf\u30f3\u30c9\u30a2\u30ed\u30f3 HTML \u30d7\u30ec\u30d3\u30e5\u30fc" onclick="event.stopPropagation()">\ud83d\udc41\ufe0f \u30d7\u30ec\u30d3\u30e5\u30fc \u2197</a>
...
y.innerHTML=g.html;window.MarkdownCompiler.renderMermaid(y)}}catch(c){y.innerHTML=`<p style="color:#ef4444;">\u30c8\u30ec\u30f3\u30c9\u53d6\u5f97\u30a8\u30e9\u30fc: ${k(c.message)}</p>`}}function k(b){return b?b.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"):""}let m="",v="monthly";const q=document.getElementById("searchInput"),L=document.getElementById("searchBtn"),x=document.getElementById("resultsGrid"),D=document.getElementById("resultsCount"),K=document.getElementById("searchTime"),
M=document.getElementById("totalPapersCount"),z=document.getElementById("paperModal"),N=document.getElementById("closeModalBtn"),O=document.getElementById("modalPaperId"),E=document.getElementById("modalPaperTitle"),F=document.getElementById("modalPaperTitleJa"),w=document.getElementById("modalPaperBody"),G=document.getElementById("modalArxivLink"),H=document.getElementById("modalPdfLink"),I=document.getElementById("modalOkfLink"),y=document.getElementById("trendContent"),A=document.getElementById("mcpToolSelect"),
t=document.getElementById("mcpArgsInput"),P=document.getElementById("runMcpBtn"),B=document.getElementById("mcpOutput");var u=new URLSearchParams(window.location.search);const J=u.get("q")||u.get("query"),C=u.get("tag")||u.get("category");C&&(m=C,document.querySelectorAll(".filter-chip").forEach(b=>{b.getAttribute("data-tag")===C?b.classList.add("active"):b.classList.remove("active")}));(async function(){try{const b=await (await fetch("/api/stats")).json();b.total_papers&&(M.textContent=Number(b.total_papers).toLocaleString())}catch(b){console.warn("Stats fetch failed",
b)}})();u=null!==J?J:"\u30da\u30f3\u30c6\u30b9\u30c8";q.value=u;a(u,!1);document.querySelectorAll(".nav-btn").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll(".nav-btn").forEach(g=>g.classList.remove("active"));document.querySelectorAll(".tab-content").forEach(g=>g.classList.remove("active"));b.classList.add("active");const c=b.getAttribute("data-tab");document.getElementById(c).classList.add("active");"trendsTab"===c&&h(v)})});document.querySelectorAll(".filter-chip").forEach(b=>
{b.addEventListener("click",()=>{document.querySelectorAll(".filter-chip").forEach(c=>c.classList.remove("active"));b.classList.add("active");m=b.getAttribute("data-tag");a(q.value,!0)})});L.addEventListener("click",()=>a(q.value,!0));q.addEventListener("keypress",b=>{"Enter"===b.key&&a(q.value,!0)});window.addEventListener("popstate",()=>{const b=new URLSearchParams(window.location.search),c=b.get("q")||"",g=b.get("tag")||"";m=g;document.querySelectorAll(".filter-chip").forEach(l=>{l.getAttribute("data-tag")===
g?l.classList.add("active"):l.classList.remove("active")});q.value=c;a(c,!1)});window.openPaperModal=async function(b){z.classList.remove("hidden");document.body.style.overflow="hidden";O.textContent=`arXiv: ${b}`;G&&(G.href=`https://arxiv.org/abs/${encodeURIComponent(b)}`);H&&(H.href=`https://arxiv.org/pdf/${encodeURIComponent(b)}.pdf`);E.textContent="\u8aad\u307f\u8fbc\u307f\u4e2d...";F.textContent="";w.innerHTML='<p class="loading-text">OKF \u30c9\u30ad\u30e5\u30e1\u30f3\u30c8\u3092\u53d6\u5f97\u4e2d...</p>';
try{const c=await (await fetch(`/api/paper/${encodeURIComponent(b)}`)).json();if("success"===c.status&&c.content){I&&c.path&&(I.href="/"+encodeURI(c.path));E.textContent=c.content.match(/title:\s*"(.*?)"/)?.[1]||b;F.textContent=c.content.match(/title_ja:\s*"(.*?)"/)?.[1]||"";const g=window.MarkdownCompiler.compile(c.content);w.innerHTML=g.html;e(b,w);window.MarkdownCompiler.renderMermaid(w)}}catch(c){w.innerHTML=`<p style="color:#ef4444;">\u53d6\u5f97\u30a8\u30e9\u30fc: ${k(c.message)}</p>`}};N.addEventListener("click",
d);window.addEventListener("keydown",b=>{"Escape"!==b.key||z.classList.contains("hidden")||d()});document.querySelectorAll(".period-btn").forEach(b=>{b.addEventListener("click",()=>{document.querySelectorAll(".period-btn").forEach(c=>c.classList.remove("active"));b.classList.add("active");v=b.getAttribute("data-period");h(v)})});A.addEventListener("change",()=>{const b=A.value;"search_security_papers"===b?t.value=JSON.stringify({query:"\u30da\u30f3\u30c6\u30b9\u30c8\u81ea\u52d5\u5316",top_k:5},null,
2):"verify_code_security"===b?t.value=JSON.stringify({code_snippet:"def login(user, pwd):\n    query = f\"SELECT * FROM users WHERE u='{user}' AND p='{pwd}'\"\n    cursor.execute(query)",language:"python"},null,2):"get_cwe_mitigation_recipe"===b?t.value=JSON.stringify({cwe_id:"CWE-89"},null,2):"get_related_papers_graph"===b?t.value=JSON.stringify({arxiv_id:"2502.16730"},null,2):"get_paper_summary"===b?t.value=JSON.stringify({arxiv_id:"2502.16730"},null,2):"get_latest_trends"===b?t.value=JSON.stringify({period:"monthly"},
null,2):"query_attack_technique"===b&&(t.value=JSON.stringify({technique_id:"T1059"},null,2))});P.addEventListener("click",async()=>{B.textContent="\u26a1 MCP JSON-RPC \u547c\u3073\u51fa\u3057\u4e2d...";try{const b=A.value,c=JSON.parse(t.value),g=await (await fetch("/api/mcp",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:b,arguments:c})})).json();B.textContent=JSON.stringify(g,null,2)}catch(b){B.textContent=`\u30a8\u30e9\u30fc: ${b.message}`}})});

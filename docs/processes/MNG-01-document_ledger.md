...
| **[DSN-23] 階層型ステートマシン (HSM) 設計書** | [designs/DSN-23-hierarchical_state_machine_and_lifecycle_governance.md](../designs/DSN-23-hierarchical_state_machine_and_lifecycle_governance.md) | ゼロ外部依存・高信頼階層型ステートマシン基盤およびシステム全域ライフサイクル統制設計。 | **Core Platform HOW** | SA, SM, SWD | SA, SWD | ライフサイクル・状態管理改訂時 |
| **[DSN-24] 統合管理 CLI & 対話型 DB シェル設計書** | [designs/DSN-24-unified_management_cli_and_interactive_database_shell.md](../designs/DSN-24-unified_management_cli_and_interactive_database_shell.md) | Django スタイル統一エントリポイント (manage.py) およびゼロ外部依存 REPL (dbshell) 設計。 | **CLI / Tooling HOW** | SA, DB, SWD | SA, DB | CLI・シェル対話拡張時 |
| **[DSN-25] 純粋 Python 製 Packrat PEG パーサーエンジン設計書** | [designs/DSN-25-pure_python_packrat_peg_parser_engine.md](../designs/DSN-25-pure_python_packrat_peg_parser_engine.md) | 線形時間 $O(N)$ 保証 Packrat PEG ランタイム、AST コンビネータ、検索/SQL/グラフDSL/オントロジー横断適用、将来の事前コード生成進化ロードマップ。 | **Core Engine HOW** | SA, SWD, IR, DB | SA, SWD | 構文解析・文法エンジン拡張時 |
| **[DSN-26] 純粋 Python 製 OWL-DL 記述論理推論エンジン設計書** | [designs/DSN-26-pure-python-owl-dl-reasoner.md](../designs/DSN-26-pure-python-owl-dl-reasoner.md) | 純粋 Python による OWL 2 DL 記述論理推論、Tableau アルゴリズム、SHACL 形状検証。 | **Reasoning HOW** | SA, SEC, SWD | SA, SEC | 推論エンジン拡張時 |
| **[DSN-27] モジュラーフロントエンドフレームワーク設計書** | [designs/DSN-27-modular_frontend_framework_and_client_architecture.md](../designs/DSN-27-modular_frontend_framework_and_client_architecture.md) | Pure-JS ゼロフレームワーククライアントアーキテクチャ、コンポーネントシステム。 | **Frontend HOW** | UI/UX, SA, APS | SA, UI/UX | フロントエンド改訂時 |
| **[DSN-28] PyNYTProf 高精度プロファイラ ＆ 可視化統合スイート設計仕様書** | [designs/DSN-28-python_nytprof_profiler_and_visualization_suite.md](../designs/DSN-28-python_nytprof_profiler_and_visualization_suite.md) | PEP 669 sys.monitoring / sys.settrace 統合、整数ナノ秒 Tick 蓄積、Flame Graph、ヒートマップ HTML 出力。 | **Performance Engineering HOW** | SA, SWD, QA, SM | SA, SWD | プロファイラ拡張時 |
| **[DSN-29] Python-LISP 統合アーキテクチャ設計仕様書 (pylisp) [改訂第6版]** | [designs/DSN-29-python_lisp_integrated_architecture_specification.md](../designs/DSN-29-python_lisp_integrated_architecture_specification.md) | CPython 3.10+ / Free-threaded 整合・contextvars 動的スコープ・現場復帰型コンディション・HAMT miniKanren・厳格採択ガード・永久棄却境界。 | **LISP Integration Architecture HOW** | SA, SWD, SEC, QA | SA, SWD | LISP統合ライブラリ改訂時 |

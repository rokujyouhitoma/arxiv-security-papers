...
| **[MNG-03] オントロジー駆動開発プロセス規範** | [processes/ontology_driven_development_process.md](ontology_driven_development_process.md) | 概念モデリングから TBox 定義、ABox 抽出、推論展開、下流同期までの 5 段階 ODD ライフサイクルおよび 13 エージェント責任分界点を定義。 | **開発プロセス規範 (Process Spec)** | 全 13 エージェント | PM, SA | オントロジー運用・開発プロセス改訂時 |

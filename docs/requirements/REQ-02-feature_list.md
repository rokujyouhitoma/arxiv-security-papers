...
```mermaid
flowchart TD
    subgraph Pipeline ["収集・標準化パイプライン (src/arxiv_okf_fetcher.py)"]
        F1["F-01: arXiv 論文自動収集"] --> F2["F-02: Google OKF v0.2 変換"]
        F2 --> F3["F-03: 5階層日本語サマリー生成"]
    end

    subgraph SearchEngine ["検索・インテリジェンスエンジン (src/vector_engine.py)"]
        F2 --> F4["F-04: 5手法統合マルチエンジン検索<br/>(Vector, BM25, Inverted, FM-Index, Recency)"]
    end

    subgraph Interfaces ["外部アクセスインターフェース"]
        F4 --> F5["F-05: MCP JSON-RPC 2.0 Server (src/mcp_server.py)"]
        F4 --> F6["F-06: Glassmorphic Web Portal (src/web_server.py, site/)"]
    end

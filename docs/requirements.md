...
### FR-03: 5階層のエグゼクティブサマリー独立生成・管理 (01_, 02_, 04_, 05_, 07_ 項番付きソート・表形式・完全日本語化対応)
それぞれのサマリー層は `outputs/executive_summaries/` 配下の項番（01, 02, 04, 05, 07）を冠したソート可能な独立ディレクトリで個別に管理されること。サマリー内の文章および論文一覧（マークダウン表形式）は **完全に日本語化 (100% Japanese)** されていること。

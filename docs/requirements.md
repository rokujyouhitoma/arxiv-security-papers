...
### FR-03: 6階層のエグゼクティブサマリー独立生成・管理 (01_, 02_, 04_〜07_ 項番付きソート・表形式・完全日本語化対応)
それぞれのサマリー層は `outputs/executive_summaries/` 配下の項番（01, 02, 04〜07）を冠したソート可能な独立ディレクトリで個別に管理されること。サマリー内の文章および論文一覧（マークダウン表形式）は **完全に日本語化 (100% Japanese)** されていること。
...
5. **半期サマリー (06_semi_annual)**:
   - 半期期間ごと（各半期末日: 06-30, 12-31 および最新実行日）、直近180日間に収集された論文群を集計した日本語表形式の半期戦略レポートを作成・更新する。

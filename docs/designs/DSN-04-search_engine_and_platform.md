...
- [7. 非機能要件・セキュリティ・リソース制約](#7-非機能要件セキュリティリソース制約)
  - [7.1 レイテンシとスループット](#71-レイテンシとスループット)
  - [7.2 セキュリティ・XSS 堅牢性](#72-セキュリティxss-堅牢性)
  - [7.3 メモリ最適化・リソース消費抑制設計（Search Worker 省メモリ化: Issue #319）](#73-メモリ最適化リソース消費抑制設計search-worker-省メモリ化-issue-319)
...
## 7.3 メモリ最適化・リソース消費抑制設計（Search Worker 省メモリ化: Issue #319）

### 7.3.1 ボトルネック特定とメモリプロファイリング
全 14,564 件の大規模セキュリティ論文インデックス展開時、Search Worker プロセスの常駐メモリ（PSS）が 1,306.4 MB に達していた課題に対し、`tracemalloc` およびメモリプロファイラによる詳細分析を実施した。特定された要因は以下の 4 点である：

1. **`MultiFieldPostingsIndex` における未使用出現位置リスト (`positions`) の肥大化**:
   - 単語ごとに `(doc_id, positions)` タプル（264万個）および出現位置整数 `positions`（484万個）をメモリ展開していた。しかしベクトルエンジンのクエリ実行ロジックでは `for doc_id, _ in get_postings(...)` と位置情報を破棄していたため、**384 MiB** が完全に浪費されていた。
2. **長文全文トークン配列の重複保持**:
   - 各ドキュメント辞書内に `tokens`（360万個）、`token_counts`（278万エントリ）、`desc_tokens`、`abstract_tokens` が重複保持され、約 **350 MiB** を消費していた。
3. **動的 `set(...)` 生成によるアロケーションオーバーヘッド**:
   - クエリ毎・候補ドキュメント毎に `set(doc.get("title_tokens", []))` を動的生成していたため、検索クエリ評価時の GC 負荷とレイテンシが増大していた。
4. **JSON シリアライズ時の冗長フィールドによるファイル・ロード時間肥大化**:
   - 上記の長大トークン配列が `index.json` にそのまま記録されていたため、ファイルサイズが **172.88 MB** に膨張。`json.load()` デコード時に一時ピークメモリ 668 MiB と 35 秒のロード遅延を誘発していた。

### 7.3.2 ゼロオーバーヘッド転置インデックス (`track_positions=False`)
`src/search/ingestion/field_schema.py` に位置情報追跡を無効化する `track_positions=False` モードを導入。
- 検索に不要な位置リスト生成をバイパスし、フィールドごとのユニーク `doc_id` リスト `self.fields: Dict[str, Dict[str, List[str]]]` 構造へ刷新。
- 単語の重複追加を即座にガードし、メモリ使用量を **384 MiB $\to$ 14.7 MiB（96.2% 削減）** に圧縮。

```python
# track_positions=False による軽量 Postings リスト管理
if not self.track_positions:
    field_dict = self.fields[field_name]
    if term not in field_dict:
        field_dict[term] = [doc_id]
    else:
        postings = field_dict[term]
        if not postings or postings[-1] != doc_id:
            postings.append(doc_id)
```

### 7.3.3 ドキュメントモデルの `frozenset` 事前コンパイルとトークンパージ
`src/search/vector_engine.py` のインメモリキャッシュ構造を抜本的に再設計。
1. **ロード時 `frozenset` 事前コンパイル**:
   - `_title_set`, `_author_set`, `_kw_set`, `_tags_set`, `_desc_set`, `_abstract_set` をロード時・インデックス構築時に一度だけ不変の `frozenset` として事前計算。
   - `_compute_token_field_score` において動的 `set()` 生成を完全撤廃し、事前コンパイル済み集合による $\mathcal{O}(1)$ 高速メンバーシップテストへ最適化。
2. **トークン配列のパージ（`_cleanup_doc_tokens`）**:
   - 転置インデックスおよび BM25 パラメータ（`avg_doc_len` 等）の算出完了後、メモリ上に不要となった `tokens`, `token_counts`, `desc_tokens`, `abstract_tokens` などの巨大配列をドキュメント辞書から解放（pop）。

### 7.3.4 スキーマ v3.5.0 永続化と動的メタデータ補完
1. **スキーマ v3.5.0**:
   - `save_index` において不要なトークンリストを除外して永続化。ディスク占有量とネットワーク転送コストを最小化。
2. **近傍グラフ動的メタデータ補完 (`_enrich_neighbor`)**:
   - `proximity_graph` の永続化ノードには余分なメタデータを持たせず、最小限のノード（`arxiv_id`, `distance` 等）のみを保持。IPC 検索サービス層（`service.py`）でレスポンス生成時にドキュメント辞書から動的にメタデータを結合。

### 7.3.5 実測パフォーマンス・省メモリ達成値
全 14,564 件の大規模本番データセットによる実測検証結果：

| 評価指標 | 最適化前 | 最適化後（v3.5.0） | 改善率 / 削減率 |
| :--- | :--- | :--- | :--- |
| **`index.json` ファイルサイズ** | 172.88 MB | **49.01 MB** | **71.7% 削減** |
| **インデックスロード時間** | 35.35 秒 | **14.89 秒** | **57.9% 高速化** |
| **転置インデックスメモリ** | 384.0 MiB | **14.7 MiB** | **96.2% 削減** |
| **Search Worker 常駐メモリ (PSS/Heap)** | 1,306.4 MB / 1,185 MB | **454.78 MB** | **61.6% 削減 (1/2.61達成)** |
| **キーワード検索レイテンシ** | 1.8 〜 3.2 秒 | **0.9 〜 2.0 秒** | **動的 set 撤廃により向上** |
| **関連論文推薦 (k-NN) レイテンシ** | 0.35 ms | **0.29 ms** | **極低遅延維持** |
| **回帰テスト合格率** | 100% (108/108) | **100% (108/108)** | **完全互換性保証** |

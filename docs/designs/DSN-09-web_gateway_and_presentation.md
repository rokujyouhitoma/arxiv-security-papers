...
- [1. Web ゲートウェイ & プレゼンテーションの全体アーキテクチャ](#1-web-ゲートウェイ--プレゼンテーションの全体アーキテクチャ)
  ...
  - [1.2 PEP 3333 WSGI 標準準拠とゼロ外部依存原則](#12-pep-3333-wsgi-標準準拠とゼロ外部依存原則)
...
## 1.2 PEP 3333 WSGI 標準準拠とゼロ外部依存原則
サードパーティの Web フレームワーク（Flask, FastAPI, Django 等）に依存せず、Python 標準ライブラリの `wsgiref.simple_server` および PEP 3333 インターフェース仕様に直接準拠し、ポータブルかつ高信頼に稼働します。

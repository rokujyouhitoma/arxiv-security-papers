...
## 4.3 引用ネットワーク統合型 GraphRAG パイプライン
抽出された STIX 知識グラフを論文間の引用関係（Citation Network）と同一のインメモリ有向グラフとして結合する。
- **引用網・脅威網結合**: 論文ノード（Paper）をハブとして、引用エッジ（`cites`）と脅威エッジ（`targets`, `mitigates`, `indicates`）を同一グラフ空間で管理。
- **マルチホップ波及追論**: PageRank や幅優先探索（BFS）による近傍走査アルゴリズムを適用し、特定の攻撃手法（例: ATT&CK T1059）を起点として、その派生攻撃を検証した論文群や、それに対抗する形式検証ツール・パッチ生成技術を提示した最新論文群を自動特定。

...
- **Metrics**: クエリ遅延（レイテンシ）、メモリ使用量、NDCG スコアの定量的集計。
  - **確率的ストリーミングメトリクス基盤 (`src/core/structures/probabilistic.py`)**:
    - **Count-Min Sketch (`CountMinSketch`)**: 独立ハッシュ関数（Murmur-style）を用いた 2 次元カウンタテーブルにより、固定メモリ領域でストリーミング頻出攻撃元、多発検索クエリ（Heavy Hitters）を誤差 $\epsilon$, 信頼度 $1-\delta$ で高速追跡。
    - **t-digest (`TDigest`)**: 動的セントロイドクラスタリングによるオンライン分位数推定アルゴリズム。膨大な全サンプルを保持することなく、P95, P99, P99.9 のテールレイテンシを極小誤差で精密算出。

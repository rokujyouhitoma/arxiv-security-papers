...
- [11. 高度ナレッジグラフ探索・フィルタリング・インタラクション機能仕様群](#11-高度ナレッジグラフ探索フィルタリングインタラクション機能仕様群)
  - [11.13 ノード非表示（Hiding）および全再表示（Unhide All）機能 (Issue #317)](#1113-ノード非表示hidingおよび全再表示unhide-all機能-issue-317)
  - [11.14 エッジ選択・関係性メタデータ＆エビデンス詳細インスペクター (Issue #318)](#1114-エッジ選択関係性メタデータエビデンス詳細インスペクター-issue-318)
...
1. **エッジ線・矢印描画**:
   - リレーション種別（`EXPLOITS`, `MITIGATES`, `DISCLOSES`, `SUBCLASS_OF` 等）に応じた破線／実線／カラーパレット描画。
   - 双方向・多重エッジにおける二次ベジェ曲線（Quadratic Bézier Curve）による曲率分離。
   - ホバー／選択エッジの強調描画（通常 1.0〜1.8px に対し、ホバー時 2.8px、選択時 3.6px のアンバーゴールド `#D97706` 強調、他エッジの自動ディミング）。
   - 終端ノード境界に合わせた動的アローヘッド描画（選択時 10px / 5.5px に拡大）。
   - エッジ中点における関係性ラベルピル（半透明背景バッジ + リレーション名 + 確信度ティア）の描画。
2. **ノード円描画**:
   - クラスタ配色フィル、境界線描画。
   - 選択エッジ接続ノード（Source / Target）に対する連動アンバー外枠（`#D97706`, 2.2px）の強調。
   - エゴネットワーク中心ノード（青色脈動リング）および未研究 Gap ノード（紫色脈動リング）のリアルタイムアニメーション。
3. **テキストラベル描画**:
   - `font-mono`, 10px, 背景コントラスト最適化、ホバー時ツールチップ案内。

## 5.3 マウスインタラクション（二層ヒットテスト、ドラッグ移動、ホバーハイライト）
- **二層ヒットテスト（Two-Tier Hit Testing）**:
  1. 第 1 優先: ノード中心とのユークリッド距離判定（$d \le r_{\text{node}} + 4$）。
  2. 第 2 優先（ノード未検出時）: `findEdgeAt(mx, my)` によるエッジ幾何最短距離判定。直線エッジ（線分射影距離）および二次ベジェ曲線エッジ（Bezier サンプリング点群探索）に対し、ワールド座標系でマウスポインタからの最短距離二乗を計算。ヒット許容閾値 $d \le \frac{8.0}{\text{scale}}\text{px}$ 内で最も近傍のエッジを抽出。
- **カーソル制御**: ノードまたはエッジのホバー時に `cursor: pointer`、キャンバス背景ホバー時に `cursor: crosshair`、パン操作時に `cursor: grabbing` を動的制御。
- **ドラッグ移動**: 該当ノードの物理移動を一時固定し、周囲ノードをバネ牽引。

## 5.4 エンティティ・リレーション統合インスペクタードロワー (`#nodeCallout`)
画面右端のフルハイトスライドイン・インスペクタードロワーにより、選択対象（ノードまたはエッジ）に応じた構造化メタデータを即座に表示：
1. **ノード選択時**:
   - クラスタタグ、接続次数（Degree）、タイトル、要約文（Description）、接続エッジリスト、エゴフォーカスボタン（`#btnFocusEgo`）、非表示ボタン（`#btnHideSelectedNode`）。
2. **エッジ選択時**:
   - リレーションタグ（`REL: <type>`, アンバー背景 `#D97706`）。
   - 確信度スコアおよびティア（`Conf: NN% (TIER)`）。
   - 接続元・接続先タイトル（`Source ➔ Target`）。
   - 推論メカニズム（`inference_mechanism`）、一次推論ルール（`primary_rule_id`）、適用推論ルール群（`applied_rules`）。
   - 原典論文からのエビデンス引用文（`evidence_quote`、左側アンバー引用ブロック）。
   - 背景クリックまたは `Escape` キー押下によるインスペクター自動クローズ。

## 5.5 第5章の要約
- 高性能 2D Canvas による軽量・軽快なインタラクティブ UI と、ノード・エッジ双方の深いインスペクション機能を実現。
...
## 11.9 エゴネットワーク（Ego Subgraph）フォーカス機能
複雑に入り組んだ大規模グラフの中から、特定ノード（論文、攻撃テクニック、CWE 等）を中心とした 1〜2 ホップの直接因果関係チェーンに完全に集中して探索するため、エゴサブグラフのフォーカス・クローズアップ表示機能を備える。

1. **UI コントロールおよび操作仕様**:
   - **ダブルクリック（Double Click）**: キャンバス上の任意のノードをダブルクリックすることで、直ちにそのノードを中心とした 2-Hop エゴネットワークフォーカス（`focusEgoNetwork(node.id, 2)`）を発動。
   - **インスペクター Drawer アクション**: サイドインスペクター（`#nodeCallout`）下部に「🎯 このノードにフォーカス (2-Hop)」（`id="btnFocusEgo"`）ボタンを配備。
   - **フォーカス中バナー**: フォーカス起動時、`mesh-toolbar` に `#focusBanner`（青色インジケーター）を表示し、中心ノード名（`#focusTargetLabel`）と「✕ 解除」（`#btnClearFocus`）ボタンを提示。
   - **フォーカス解除（Deselect / Clear）**: `#btnClearFocus` の押下、またはキャンバスの空き領域（背景）をクリックすることで即座に全域表示（`clearNodeFocus()`）へ復帰。

2. **アルゴリズムと循環参照ガード（BFS Loop Guard）**:
   - 状態変数 `focusedNodeId`（中心ノードID）および `focusedHopNodeIds`（Set of reachable Node IDs）を管理。
   - `getEgoNeighborhood(centerId, depth = 2)`:
     - 探索深度 $d$ を $1 \le d \le 3$ に強制クランプし、極端な多ホップ展開による計算バーストを抑止。
     - 幅優先探索（BFS）を実施し、訪問済みノードを `visited` Set で追跡することで、相互参照や閉ループを持つグラフ構造でも無限ループ・スタックオーバーフローを 100% 防止。
   - `graphQueryResultBadge` に `🎯 エゴフォーカス: [Node] (関連 N ノード)` をリアルタイム反映。

3. **描画パイプライン（Rendering Pipeline）統合**:
   - `render()` 描画ループにおいて `focusedHopNodeIds` を参照：
     - **非対象ノード**: `ctx.globalAlpha = 0.08` に大幅減衰（ディミング）。
...
## 11.13 ノード非表示（Hiding）および全再表示（Unhide All）機能 (Issue #317)
グラフ探索中に特定の関心外ノードや巨大ハブノードを一時的にキャンバスから排除し、残存ネットワークのトポロジー分析に専念できるノード非表示制御機能。

1. **状態管理とセッション永続化**:
   - `hiddenNodeIds = new Set()`（非表示ノードID集合）を管理。
   - `hideNode(nodeId)`: ノードを集合に追加し、選択解除およびコールアウト閉塞を実行。
   - `unhideAllNodes()`: 集合を全クリアし、キャンバスを完全復元。
   - `localStorage` による `dashboard_hidden_nodes` の自動永続化（リロード後も状態を維持）。
2. **UI コントロールおよびキーバインド**:
   - インスペクター Drawer（`#nodeCallout`）下部に「👁️ このノードを非表示」（`#btnHideSelectedNode`）ボタンを配備。
   - ツールバーおよびステータスバーに「👁️ 全再表示 (N件非表示)」（`#btnUnheatHiddenNodes` / `#btnUnhideAll`）ボタンを動的表示。
   - ショートカットキー: `Delete` / `Backspace` / `X` で選択中ノードを非表示、`Shift + H` または `U` で全再表示。
3. **フィルタパイプライン統合**:
   - `applyCtiFilter()` および `applyContextMesh()` において `!hiddenNodeIds.has(n.id)` をフィルタ先頭で適用。
   - 非表示ノードに接続するエッジ群も自動的に除外され、力学モデル演算および次数再計算から完全に分離。

## 11.14 エッジ選択・関係性メタデータ＆エビデンス詳細インスペクター (Issue #318)
ノードだけでなくエッジ（リレーション）自体を直接クリックして選択可能とし、エッジが保持する因果関係メタデータ、推論ルール、および原典学術論文からのエビデンス引用文を詳細インスペクターに展開する機能。

1. **幾何学的最短距離判定（Bezier & Segment Distance）**:
   - `findEdgeAt(mx, my)`:
     - 直線エッジ: 線分への正射影ベクトルを用いた最短距離二乗（`pointToSegmentDistanceSq`）。
     - 二次ベジェ曲線エッジ（双方向・多重エッジ）: 曲率制御点 $(cx, cy)$ を算出し、Bezier 方程式 $B(t) = (1-t)^2 u + 2(1-t)t c + t^2 v$ 上の 7 点サンプリングによる最短距離二乗近似。
     - ヒット閾値: $\text{distance} \le \frac{8.0}{\text{viewTransform.scale}}\text{px}$（ズーム率依存の画面 8px 判定）。
     - ノード優先原則: ノードヒットテスト（`findNodeAt`）がエッジ判定に常に先行し、ノードのドラッグ操作と干渉しない。
2. **選択状態のライフサイクル管理**:
   - `selectedEdge`: 現在選択中のエッジオブジェクト（排他的に `selectedNode = null`）。
   - `hoveredEdge`: マウスオーバー中のエッジ（カーソルを `pointer` に切り替え）。
   - `selectEdge(edge)`:
     - エッジ接続元・接続先ノード情報（`nodeMap.get(edge.source)`, `nodeMap.get(edge.target)`）を解決。
     - インスペクター Drawer（`#nodeCallout`）にリレーション名（アンバータグ `#D97706`）、確信度スコア/ティア、推論メカニズム（`inference_mechanism`）、一次ルール（`primary_rule_id`）、適用推論ルール一覧（`applied_rules`）、および原典エビデンス引用文（`evidence_quote`）をレンダリング。
     - ノード固有ボタン（`#btnFocusEgo`, `#btnHideSelectedNode`）を非表示化。
   - `closeCallout()` / 背景クリック / `Escape` キー押下による選択解除。
3. **レンダリングパイプライン統合**:
   - 選択エッジ描画: `ctx.strokeStyle = '#D97706'`、`ctx.lineWidth = 3.6`、アローヘッドサイズ拡大（10.0px / 5.5px）。
   - ホバーエッジ描画: `ctx.strokeStyle = '#F59E0B'`、`ctx.lineWidth = 2.8`。
   - 非選択エッジの自動ディミング: 選択エッジが存在する場合、無関係な他のエッジを `globalAlpha = 0.20` に自動減衰。
   - エッジ両端ノードの強調外枠: 接続元・接続先ノードの外周にアンバーリング（`#D97706`, 2.2px）を描画。
   - 関係性ラベルピル: 選択・ホバー時にエッジ中点へリレーション名と確信度ティアを含む半透明ピルバッジを自動展開。

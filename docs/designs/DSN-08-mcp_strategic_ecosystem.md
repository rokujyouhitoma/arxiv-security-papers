...
- [1. MCP エコシステムと戦略的ビジョン](#1-mcp-エコシステムと戦略的ビジョン)
  ...
  - [1.3 ゼロ外部依存原則と標準 Python 実装](#13-ゼロ外部依存原則と標準-python-実装)
...
## 1.3 ゼロ外部依存原則と標準 Python 実装
外部フレームワークを一切使用せず、Python 3.11+ 標準ライブラリ（`json`, `sys`, `typing`, `dataclasses`, `cProfile`, `tracemalloc`, `dis` 等）のみで完全自律稼働します。

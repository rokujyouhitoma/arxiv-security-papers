...
`src/settings.py` の末尾には、設定モジュールを `src/core/settings/` へ分割した際に過渡期措置として設置された動的フォールバック `__getattr__` が残存している：
```python
...
この動的委譲ハックは、静的型チェッカー（mypy）や IDE による補完・検証の精度を低下させ、コードの追跡性を損ねている。
本 Issue では、プロジェクト全体で `src.settings` 経由でこれらの関数を呼び出している箇所を正規の `src.core.settings` 直接インポートに置き換え、`__getattr__` を撤去して設定アーキテクチャの多重構造を一本化する。
...
- 関連コード: `src/settings.py`, `src/core/settings/`
- 関連品質ゲート: `make static_analysis` (mypy strict compliance)
...
### 設定モジュール
- [ ] `src/settings.py` (`__getattr__` 削除)
- [ ] `src/core/settings/`
...
- [ ] `src/settings.py` から `__getattr__` フォールバックが完全に削除されていること。
- [ ] プロジェクト全体で設定関数のインポートが正規の `core.settings` に統一されていること。
- [ ] 静的型検査（mypy）およびテストスイートが 100% PASS すること。

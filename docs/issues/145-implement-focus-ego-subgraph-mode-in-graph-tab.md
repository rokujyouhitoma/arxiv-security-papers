...
- [ ] ノード詳細 Drawer に「🎯 このノードにフォーカス」ボタンが配置されていること。
- [ ] ノードのダブルクリックまたはボタン押下で、該当ノードから2ホップ以内のエゴサブグラフのみがクローズアップ表示され、他ノードがフェードアウトすること。
- [ ] ツールバーに「✕ 解除」ボタンが表示され、クリックまたは背景クリックで通常表示へ即座に復帰すること。
- [ ] 循環グラフ構造でも BFS が安全に停止しブラウザがフリーズしないこと。

...
### ストレージ・データ
- [ ] `outputs/okf/papers/**/CWE-*.md` (2,832件の削除対象)
- [ ] `outputs/okf/cwes/` (差分マージ先)

### パイプライン・バックエンド
- [ ] `src/pipeline/arxiv_okf_fetcher.py` (CWE混入防止ガード)
- [ ] `src/domain/security/pipeline/okf_pipeline.py` (出力パス判定の厳格化)
- [ ] `src/settings.py` (`okf_papers`, `okf_cwes` テーブル定義)

### テストスイート
- [ ] `tests/domain/test_okf_pipeline.py`
- [ ] `tests/pipeline/test_arxiv_okf_fetcher.py`
...
- [ ] `outputs/okf/papers/` 配下の `CWE-*.md` 件数が 0 件であること。
- [ ] `outputs/okf/cwes/` に必要な CWE OKF ファイルが欠落なく保全されていること。
- [ ] 論文取得パイプラインで CWE ドキュメントが `papers` 配下に出力されないガードが機能していること。
- [ ] 全ユニットテストおよび静的解析（mypy, xenon rank A）が 100% PASS すること。

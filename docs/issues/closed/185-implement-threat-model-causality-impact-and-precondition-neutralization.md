...
- 関連Issue: [Issue 184](closed/184-enhance-owl-logic-incident-coupling-and-standards-alignment.md)

---

## 3. 影響範囲と関連ファイル / Scope and Affected Files
- [x] [src/ontology/turtle_engine.py](../../src/ontology/turtle_engine.py) (オントロジー定義)
- [x] [src/ontology/schema.py](../../src/ontology/schema.py) (スキーマ定義)
...
3. **データ型プロパティ定義**:
   ...
   - `sec:impactSeverity` (Domain: `sec:Impact`, Range: `xsd:string`, 例: "Critical", "High")
4. **テスト作成 & 検証**:
...
- [x] `sec:Impact` クラスおよび `sec:hasImpact`, `sec:neutralizesPrecondition` 述語が定義されていること。
- [x] すべてのプロパティに適切な逆関係 (`owl:inverseOf`) が設定されていること。
- [x] STRIDE 分類用データ型プロパティが定義されていること。
- [x] テストが全件 PASS すること。

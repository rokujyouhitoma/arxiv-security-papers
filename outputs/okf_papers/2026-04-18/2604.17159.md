---
type: "security-paper"
title: "Systematic Capability Benchmarking of Frontier Large Language Models for Offensive Cyber Tasks"
title_ja: "Systematic Capability of Frontier Large Language Models for Offensive Cyber Tasksのベンチマーク評価"
description: "Systematic Capability of Frontier Large Language Models for Offensive Cyber Tasksのベンチマーク評価 — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.17159v1"
tags:
  - "cs.AI"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "cs.CL"
  - "arXiv"
timestamp: "2026-08-14T06:11:06Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-18/2604.17159_meta.json"
  published_date: "2026-04-18T22:13:23Z"
  authors:
    - "Tyler H. Merves"
    - "Michael H. Conaway"
    - "Joseph M. Escobar"
    - "Hakan T. Otal"
    - "Unal Tatar"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Systematic Capability Benchmarking of Frontier Large Language Models for Offensive Cyber Tasks
### (日本語題名: Systematic Capability of Frontier Large Language Models for Offensive Cyber Tasksのベンチマーク評価)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.17159v1`](https://arxiv.org/abs/2604.17159v1) | Raw Meta = [`2604.17159_meta.json`](../../raw_data/2026-04-18/2604.17159_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Systematic Capability of Frontier Large Language Models for Offensive Cyber Tasksのベンチマーク評価」（原題: Systematic Capability Benchmarking of Frontier Large Language Models for Offensive Cyber Tasks / arXiv: 2604.17159v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.17159v1`
- **論文URL**: [https://arxiv.org/abs/2604.17159v1](https://arxiv.org/abs/2604.17159v1)
- **PDFリンク**: [https://arxiv.org/pdf/2604.17159v1.pdf](https://arxiv.org/pdf/2604.17159v1.pdf)
- **著者**: Tyler H. Merves, Michael H. Conaway, Joseph M. Escobar, Hakan T. Otal, Unal Tatar
- **公開日時**: `2026-04-18T22:13:23Z`
- **カテゴリ**: `cs.CR, cs.AI, cs.CL`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-18/2604.17159_meta.json) | [`PDF`](../../raw_data/2026-04-18/2604.17159.pdf) | [`TXT`](../../raw_data/2026-04-18/2604.17159.txt)

### Abstract (原文)
> We present, to our knowledge, the most comprehensive cross-model evaluation of LLM agents on offensive cybersecurity tasks, benchmarking 10 frontier models from 7 providers on all 200 challenges of the NYU CTF Bench. Building on the D-CIPHER multi-agent framework, we extend it with multi-provider backend support, a custom Kali Linux environment with over 100 pre-installed penetration testing tools, and runtime tool-discovery agents. Through a controlled factorial study, we find that the Kali Linux environment yields a +9.5 percentage-point improvement over Ubuntu, while auto-prompting and category-specific tips often degrade performance in well-equipped environments. Among models, Claude 4.5 Opus achieves the highest solve rate (59%), followed by Gemini 3 Pro (52%), with Gemini 3 Flash offering the best cost-efficiency at $0.05 per solve. Asymmetric planner/executor model assignments provide no meaningful benefit while coherent same-model configurations consistently outperform mixed-tier pairings. Our results indicate that environment tooling and model selection emerge as the strongest drivers of performance, whereas prompt engineering interventions show diminishing or negative returns in well-equipped environments. Reported performance reflects both model reasoning ability and compatibility with agent tooling and API integration.

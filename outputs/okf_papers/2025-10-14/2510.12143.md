---
type: "security-paper"
title: "Fairness-Constrained Optimization Attack in Federated Learning"
title_ja: "Fairness-Constrained Optimization Attack in 連合学習"
description: "Fairness-Constrained Optimization Attack in 連合学習 — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2510.12143v1"
tags:
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
timestamp: "2026-08-14T06:11:10Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-10-14/2510.12143_meta.json"
  published_date: "2025-10-14T04:49:53Z"
  authors:
    - "Harsh Kasyap"
    - "Minghong Fang"
    - "Zhuqing Liu"
    - "Carsten Maple"
    - "Somanath Tripathy"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Fairness-Constrained Optimization Attack in Federated Learning
### (日本語題名: Fairness-Constrained Optimization Attack in 連合学習)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2510.12143v1`](https://arxiv.org/abs/2510.12143v1) | Raw Meta = [`2510.12143_meta.json`](../../raw_data/2025-10-14/2510.12143_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Fairness-Constrained Optimization Attack in 連合学習」（原題: Fairness-Constrained Optimization Attack in Federated Learning / arXiv: 2510.12143v1）は、cs.LG 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, privacy)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, privacy)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2510.12143v1`
- **論文URL**: [https://arxiv.org/abs/2510.12143v1](https://arxiv.org/abs/2510.12143v1)
- **PDFリンク**: [https://arxiv.org/pdf/2510.12143v1.pdf](https://arxiv.org/pdf/2510.12143v1.pdf)
- **著者**: Harsh Kasyap, Minghong Fang, Zhuqing Liu, Carsten Maple, Somanath Tripathy
- **公開日時**: `2025-10-14T04:49:53Z`
- **カテゴリ**: `cs.LG, cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-10-14/2510.12143_meta.json) | [`PDF`](../../raw_data/2025-10-14/2510.12143.pdf) | [`TXT`](../../raw_data/2025-10-14/2510.12143.txt)

### Abstract (原文)
> Federated learning (FL) is a privacy-preserving machine learning technique that facilitates collaboration among participants across demographics. FL enables model sharing, while restricting the movement of data. Since FL provides participants with independence over their training data, it becomes susceptible to poisoning attacks. Such collaboration also propagates bias among the participants, even unintentionally, due to different data distribution or historical bias present in the data. This paper proposes an intentional fairness attack, where a client maliciously sends a biased model, by increasing the fairness loss while training, even considering homogeneous data distribution. The fairness loss is calculated by solving an optimization problem for fairness metrics such as demographic parity and equalized odds. The attack is insidious and hard to detect, as it maintains global accuracy even after increasing the bias. We evaluate our attack against the state-of-the-art Byzantine-robust and fairness-aware aggregation schemes over different datasets, in various settings. The empirical results demonstrate the attack efficacy by increasing the bias up to 90\%, even in the presence of a single malicious client in the FL system.

---
type: "security-paper"
title: "EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning"
title_ja: "EBS-CFL: Efficient and Byzantine-robust Secure Clustered 連合学習"
description: "EBS-CFL: Efficient and Byzantine-robust Secure Clustered 連合学習 — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2506.13612v1"
tags:
  - "cs.CR"
  - "arXiv"
  - "cybersecurity"
  - "cs.DC"
  - "executive-summary"
  - "cs.AI"
timestamp: "2026-08-14T07:15:31Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-06-16/2506.13612_meta.json"
  published_date: "2025-06-16T15:39:10Z"
  authors:
    - "Zhiqiang Li"
    - "Haiyong Bao"
    - "Menghong Guan"
    - "Hao Pan"
    - "Cheng Huang"
    - "Hong-Ning Dai"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning
### (日本語題名: EBS-CFL: Efficient and Byzantine-robust Secure Clustered 連合学習)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2506.13612v1`](https://arxiv.org/abs/2506.13612v1) | Raw Meta = [`2506.13612_meta.json`](../../raw_data/2025-06-16/2506.13612_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「EBS-CFL: Efficient and Byzantine-robust Secure Clustered 連合学習」（原題: EBS-CFL: Efficient and Byzantine-robust Secure Clustered Federated Learning / arXiv: 2506.13612v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, privacy, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, privacy, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2506.13612v1`
- **論文URL**: [https://arxiv.org/abs/2506.13612v1](https://arxiv.org/abs/2506.13612v1)
- **PDFリンク**: [https://arxiv.org/pdf/2506.13612v1.pdf](https://arxiv.org/pdf/2506.13612v1.pdf)
- **著者**: Zhiqiang Li, Haiyong Bao, Menghong Guan, Hao Pan, Cheng Huang, Hong-Ning Dai
- **公開日時**: `2025-06-16T15:39:10Z`
- **カテゴリ**: `cs.CR, cs.AI, cs.DC`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-06-16/2506.13612_meta.json) | [`PDF`](../../raw_data/2025-06-16/2506.13612.pdf) | [`TXT`](../../raw_data/2025-06-16/2506.13612.txt)

### Abstract (原文)
> Despite federated learning (FL)'s potential in collaborative learning, its performance has deteriorated due to the data heterogeneity of distributed users. Recently, clustered federated learning (CFL) has emerged to address this challenge by partitioning users into clusters according to their similarity. However, CFL faces difficulties in training when users are unwilling to share their cluster identities due to privacy concerns. To address these issues, we present an innovative Efficient and Robust Secure Aggregation scheme for CFL, dubbed EBS-CFL. The proposed EBS-CFL supports effectively training CFL while maintaining users' cluster identity confidentially. Moreover, it detects potential poisonous attacks without compromising individual client gradients by discarding negatively correlated gradients and aggregating positively correlated ones using a weighted approach. The server also authenticates correct gradient encoding by clients. EBS-CFL has high efficiency with client-side overhead O(ml + m^2) for communication and O(m^2l) for computation, where m is the number of cluster identities, and l is the gradient size. When m = 1, EBS-CFL's computational efficiency of client is at least O(log n) times better than comparison schemes, where n is the number of clients.In addition, we validate the scheme through extensive experiments. Finally, we theoretically prove the scheme's security.

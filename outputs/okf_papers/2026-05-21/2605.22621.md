---
type: "security-paper"
title: "UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection"
title_ja: "UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection（セキュリティ分析論文）"
description: "UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2605.22621v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.NI"
  - "cs.LG"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:54Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-05-21/2605.22621_meta.json"
  published_date: "2026-05-21T15:31:10Z"
  authors:
    - "Saif Alzubi"
    - "Frederic Stahl"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection
### (日本語題名: UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2605.22621v1`](https://arxiv.org/abs/2605.22621v1) | Raw Meta = [`2605.22621_meta.json`](../../raw_data/2026-05-21/2605.22621_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection（セキュリティ分析論文）」（原題: UNAD+: An Explainable Hybrid Framework for Unknown Network Attack Detection / arXiv: 2605.22621v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, zero-day)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, zero-day)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2605.22621v1`
- **論文URL**: [https://arxiv.org/abs/2605.22621v1](https://arxiv.org/abs/2605.22621v1)
- **PDFリンク**: [https://arxiv.org/pdf/2605.22621v1.pdf](https://arxiv.org/pdf/2605.22621v1.pdf)
- **著者**: Saif Alzubi, Frederic Stahl
- **公開日時**: `2026-05-21T15:31:10Z`
- **カテゴリ**: `cs.CR, cs.LG, cs.NI`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-05-21/2605.22621_meta.json) | [`PDF`](../../raw_data/2026-05-21/2605.22621.pdf) | [`TXT`](../../raw_data/2026-05-21/2605.22621.txt)

### Abstract (原文)
> The detection of previously unseen network attacks remains a major challenge for intrusion detection systems. Although supervised learning methods often perform well on known attack classes, they are limited when new attack types are not represented in the training data. Unsupervised methods are more suitable for detecting zero-day attacks, as they do not require labelled attack samples, but they often suffer from high false positive rates, which limits their real-world usefulness. This paper presents UNAD+, an enhanced framework for unknown network attack detection derived from the previously proposed Unknown Network Attack Detector (UNAD). UNAD+ combines a benign-only unsupervised ensemble with Weighted Majority Voting (WMV), a supervised refinement stage trained on pseudo-labelled detections, and a post hoc explainability layer that provides both local and global explanations. The framework was evaluated on the CICIDS2017 and NSL-KDD benchmark datasets. The results show that UNAD+ improves on the original UNAD framework, achieving F1-scores above 98% across the benchmark datasets while significantly reducing false positives and enhancing transparency and deployment suitability through integrated explainability.

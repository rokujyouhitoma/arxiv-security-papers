---
type: "security-paper"
title: "Your Privacy My Cloak: Backdoor Attacks on Differentially Private Federated Learning"
title_ja: "Your Privacy My Cloak: Backdoor Attacks on Differentially Private 連合学習"
description: "Your Privacy My Cloak: Backdoor Attacks on Differentially Private 連合学習 — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2606.17035v2"
tags:
  - "AI/ML Security"
  - "arXiv"
  - "cs.CR"
  - "cs.LG"
  - "cybersecurity"
  - "executive-summary"
  - "privacy-preservation"
timestamp: "2026-08-20T20:42:01Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-06-15/2606.17035_meta.json"
  published_date: "2026-06-15T17:53:12Z"
  authors:
    - "Xiaolin Li"
    - "Ning Wang"
    - "Ninghui Li"
    - "Wenhai Sun"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Your Privacy My Cloak: Backdoor Attacks on Differentially Private Federated Learning
### (日本語題名: Your Privacy My Cloak: Backdoor Attacks on Differentially Private 連合学習)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2606.17035v2`](https://arxiv.org/abs/2606.17035v2) | Raw Meta = [`2606.17035_meta.json`](../../raw_data/2026-06-15/2606.17035_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Your Privacy My Cloak: Backdoor Attacks on Differentially Private 連合学習」（原題: Your Privacy My Cloak: Backdoor Attacks on Differentially Private Federated Learning / arXiv: 2606.17035v2）は、cs.LG 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat, exploit)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat, exploit)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2606.17035v2`
- **論文URL**: [https://arxiv.org/abs/2606.17035v2](https://arxiv.org/abs/2606.17035v2)
- **PDFリンク**: [https://arxiv.org/pdf/2606.17035v2.pdf](https://arxiv.org/pdf/2606.17035v2.pdf)
- **著者**: Xiaolin Li, Ning Wang, Ninghui Li, Wenhai Sun
- **公開日時**: `2026-06-15T17:53:12Z`
- **カテゴリ**: `cs.LG, cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-06-15/2606.17035_meta.json) | [`PDF`](../../raw_data/2026-06-15/2606.17035.pdf) | [`TXT`](../../raw_data/2026-06-15/2606.17035.txt)

### Abstract (原文)
> Prior research suggests that differential privacy (DP) inherently enhances the robustness of federated learning (FL) against backdoor attacks. In this paper, we challenge this assumption. Through an empirical analysis of two baseline attack strategies, we uncover a fundamental tension in DP-FL: while bypassing DP allows state-of-the-art defenses to detect and filter malicious updates, complying with DP inadvertently masks their distinguishing statistical characteristics. Consequently, existing defenses become ineffective as DP reduces the raw backdoor signal. Building on this masking effect, we propose RING, a novel attack that explicitly exploits DP to conceal malicious contributions while maximizing attack impact. By collaboratively crafting adversarial perturbations, compromised clients reconstruct a strong backdoor signal during aggregation without triggering anomaly detection. RING operates as a perturbation layer that is agnostic to the underlying backdoor technique, making it broadly applicable and composable with existing attacks -- a property that significantly amplifies the threat it poses to DP-FL. Extensive evaluations across four image and text datasets under non-iid distributions show that RING achieves an average attack success rate of 90.3% against six state-of-the-art defenses under a moderate privacy budget, an improvement of up to 26.08x over baseline strategies. Finally, we evaluate potential countermeasures and find that mitigating this threat incurs significant utility trade-offs, exposing a fundamental security gap in the deployment of differentially private FL.

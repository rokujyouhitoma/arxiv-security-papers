---
type: "security-paper"
title: "Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks"
title_ja: "Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks（セキュリティ分析論文）"
description: "Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2606.20893v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.AI"
  - "cs.SD"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:55Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-06-18/2606.20893_meta.json"
  published_date: "2026-06-18T19:40:46Z"
  authors:
    - "Sameek Bhattacharya"
    - "Bharath Krishnamurthy"
    - "Ajita Rattani"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks
### (日本語題名: Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2606.20893v1`](https://arxiv.org/abs/2606.20893v1) | Raw Meta = [`2606.20893_meta.json`](../../raw_data/2026-06-18/2606.20893_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks（セキュリティ分析論文）」（原題: Exploiting Neural Audio Codec Latents for Adversarial Audio Attacks / arXiv: 2606.20893v1）は、cs.SD 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2606.20893v1`
- **論文URL**: [https://arxiv.org/abs/2606.20893v1](https://arxiv.org/abs/2606.20893v1)
- **PDFリンク**: [https://arxiv.org/pdf/2606.20893v1.pdf](https://arxiv.org/pdf/2606.20893v1.pdf)
- **著者**: Sameek Bhattacharya, Bharath Krishnamurthy, Ajita Rattani
- **公開日時**: `2026-06-18T19:40:46Z`
- **カテゴリ**: `cs.SD, cs.AI, cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-06-18/2606.20893_meta.json) | [`PDF`](../../raw_data/2026-06-18/2606.20893.pdf) | [`TXT`](../../raw_data/2026-06-18/2606.20893.txt)

### Abstract (原文)
> Deep learning-based audio classification systems, including automatic speaker verification, are vulnerable to adversarial attacks. Realistic real-time threat assessment remains difficult because optimization-based methods, such as projected gradient descent (PGD) and Carlini-Wagner, require costly iterative updates in the high-dimensional waveform domain. Generative attacks allow single-shot synthesis but often introduce perceptible artifacts or depend on computationally intensive architectures, while diffusion and autoregressive approaches incur high inference latency. To address this gap, we propose a generative attack framework operating in the continuous latent space of a neural audio codec. A conditional generator synthesizes class-specific perturbations in a single forward pass and decodes them into adversarial waveforms. Our method achieves targeted attack success rates up to 99% with sub-7 ms inference, outperforming generative baselines while reducing latency by 24x.

---
type: "security-paper"
title: "SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation"
title_ja: "SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation（セキュリティ分析論文）"
description: "SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.27238v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.CR"
  - "cs.AR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:55Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-29/2604.27238_meta.json"
  published_date: "2026-04-29T22:26:00Z"
  authors:
    - "Mahshid Rezakhani"
    - "Nowfel Mashnoor"
    - "Kimia Azar"
    - "Hadi Kamali"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation
### (日本語題名: SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.27238v1`](https://arxiv.org/abs/2604.27238v1) | Raw Meta = [`2604.27238_meta.json`](../../raw_data/2026-04-29/2604.27238_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation（セキュリティ分析論文）」（原題: SafeTune: Mitigating Data Poisoning in LLM Fine-Tuning for RTL Code Generation / arXiv: 2604.27238v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.27238v1`
- **論文URL**: [https://arxiv.org/abs/2604.27238v1](https://arxiv.org/abs/2604.27238v1)
- **PDFリンク**: [https://arxiv.org/pdf/2604.27238v1.pdf](https://arxiv.org/pdf/2604.27238v1.pdf)
- **著者**: Mahshid Rezakhani, Nowfel Mashnoor, Kimia Azar, Hadi Kamali
- **公開日時**: `2026-04-29T22:26:00Z`
- **カテゴリ**: `cs.CR, cs.AR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-29/2604.27238_meta.json) | [`PDF`](../../raw_data/2026-04-29/2604.27238.pdf) | [`TXT`](../../raw_data/2026-04-29/2604.27238.txt)

### Abstract (原文)
> As large language models (LLMs) are increasingly fine-tuned for hardware tasks like RTL code generation, the scarcity of high-quality datasets often leads to the use of rapidly assembled or generated training data. These datasets frequently lack security verification and are highly susceptible to data poisoning attacks. Such poisoning can cause models to generate syntactically valid but insecure hardware modules that bypass standard functionality checks. To address this, we present SafeTune, a framework designed to harden LLM-based RTL generation against poisoning, specifically focusing on hardware Trojan (HT) insertion. SafeTune integrates two core components: (i) a Graph Neural Network (GNN) that models structural properties to identify anomalous circuitry patterns during fine-tuning, and (ii) a semantic verification module using text embeddings and an XGBoost classifier to assess prompt security. By coupling structural and semantic knowledge, SafeTune effectively filters poisoned inputs without sacrificing legitimate data. Experimental results demonstrate that SafeTune significantly enhances the robustness and reliability of LLM fine-tuning without requiring modifications to the underlying model architecture.

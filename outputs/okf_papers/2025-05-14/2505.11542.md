---
type: "security-paper"
title: "Cybersecurity threat detection based on a UEBA framework using Deep Autoencoders"
title_ja: "サイバーセキュリティ threat detection based on a UEBA framework using Deep Autoencoders"
description: "サイバーセキュリティ threat detection based on a UEBA framework using Deep Autoencoders — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2505.11542v2"
tags:
  - "cs.CR"
  - "arXiv"
  - "stat.CO"
  - "cs.LG"
  - "cybersecurity"
  - "executive-summary"
  - "stat.ML"
timestamp: "2026-08-14T07:15:32Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-05-14/2505.11542_meta.json"
  published_date: "2025-05-14T13:18:12Z"
  authors:
    - "Jose Fuentes"
    - "Ines Ortega-Fernandez"
    - "Nora M. Villanueva"
    - "Marta Sestelo"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Cybersecurity threat detection based on a UEBA framework using Deep Autoencoders
### (日本語題名: サイバーセキュリティ threat detection based on a UEBA framework using Deep Autoencoders)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2505.11542v2`](https://arxiv.org/abs/2505.11542v2) | Raw Meta = [`2505.11542_meta.json`](../../raw_data/2025-05-14/2505.11542_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「サイバーセキュリティ threat detection based on a UEBA framework using Deep Autoencoders」（原題: Cybersecurity threat detection based on a UEBA framework using Deep Autoencoders / arXiv: 2505.11542v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat, leak)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat, leak)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2505.11542v2`
- **論文URL**: [https://arxiv.org/abs/2505.11542v2](https://arxiv.org/abs/2505.11542v2)
- **PDFリンク**: [https://arxiv.org/pdf/2505.11542v2.pdf](https://arxiv.org/pdf/2505.11542v2.pdf)
- **著者**: Jose Fuentes, Ines Ortega-Fernandez, Nora M. Villanueva, Marta Sestelo
- **公開日時**: `2025-05-14T13:18:12Z`
- **カテゴリ**: `cs.CR, cs.LG, stat.CO, stat.ML`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-05-14/2505.11542_meta.json) | [`PDF`](../../raw_data/2025-05-14/2505.11542.pdf) | [`TXT`](../../raw_data/2025-05-14/2505.11542.txt)

### Abstract (原文)
> User and Entity Behaviour Analytics (UEBA) is a broad branch of data analytics that attempts to build a normal behavioural profile in order to detect anomalous events. Among the techniques used to detect anomalies, Deep Autoencoders constitute one of the most promising deep learning models on UEBA tasks, allowing explainable detection of security incidents that could lead to the leak of personal data, hijacking of systems, or access to sensitive business information. In this study, we introduce the first implementation of an explainable UEBA-based anomaly detection framework that leverages Deep Autoencoders in combination with Doc2Vec to process both numerical and textual features. Additionally, based on the theoretical foundations of neural networks, we offer a novel proof demonstrating the equivalence of two widely used definitions for fully-connected neural networks. The experimental results demonstrate the proposed framework capability to detect real and synthetic anomalies effectively generated from real attack data, showing that the models provide not only correct identification of anomalies but also explainable results that enable the reconstruction of the possible origin of the anomaly. Our findings suggest that the proposed UEBA framework can be seamlessly integrated into enterprise environments, complementing existing security systems for explainable threat detection.

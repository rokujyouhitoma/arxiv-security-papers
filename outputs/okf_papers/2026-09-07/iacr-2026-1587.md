...
description: "【提案】The algorithm constructs a random prime-index superlattice $\\Gamma\\supset \\mathcal{L}$ ...。実証評価によりThen it simply scans the ..."
...
本論文「Solving the Shortest Vector Problem in $2^{0.7314n+o(n)}$ Time via Discrete Gaussian Sampling on Superlattices（セキュリティ分析論文）」（原題: Solving the Shortest Vector Problem in $2^{0.7314n+o(n)}$ Time via Discrete Gaussian Sampling on Superlattices / IACR: 2026-1587）は、cryptography 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 【提案】The algorithm constructs a random prime-index superlattice $\Gamma\supset \mathcal{L}$ ...。実証評価によりThen it simply scans the ...

### 2. 背景とセキュリティ上の課題 (Background & Problem)
【課題・脅威】We give a classical randomized algorithm for the Euclidean Shortest Vector Problem (SVP...
(主要キーワード: Euclidean Shortest Vector Problem, Shortest Vector Problem, Discrete Gaussian Sampling, full-rank lattices)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
【提案技術】The algorithm constructs a random prime-index superlattice $\Gamma\supset \mathcal{L}$ ...

### 4. セキュリティ影響と実験結果 (Results & Impact)
【実証・影響】Then it simply scans the resulting samples and retains the shortest nonzero one that li...
...
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-09-02/iacr-2026-1587_meta.json) | [`PDF (arXiv)`](https://eprint.iacr.org/2026/1587.pdf) | [`TXT`](../../raw_data/2026-09-02/iacr-2026-1587_meta.json)

---
type: "security-paper"
title: "Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model"
title_ja: "Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model（セキュリティ分析論文）"
description: "Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2503.10944v1"
tags:
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
  - "cybersecurity"
  - "executive-summary"
timestamp: "2026-08-14T07:15:32Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-03-13/2503.10944_meta.json"
  published_date: "2025-03-13T23:03:09Z"
  authors:
    - "SE Blake"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model
### (日本語題名: Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2503.10944v1`](https://arxiv.org/abs/2503.10944v1) | Raw Meta = [`2503.10944_meta.json`](../../raw_data/2025-03-13/2503.10944_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model（セキュリティ分析論文）」（原題: Phishsense-1B: A Technical Perspective on an AI-Powered Phishing Detection Model / arXiv: 2503.10944v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: threat, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: threat, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2503.10944v1`
- **論文URL**: [https://arxiv.org/abs/2503.10944v1](https://arxiv.org/abs/2503.10944v1)
- **PDFリンク**: [https://arxiv.org/pdf/2503.10944v1.pdf](https://arxiv.org/pdf/2503.10944v1.pdf)
- **著者**: SE Blake
- **公開日時**: `2025-03-13T23:03:09Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-03-13/2503.10944_meta.json) | [`PDF`](../../raw_data/2025-03-13/2503.10944.pdf) | [`TXT`](../../raw_data/2025-03-13/2503.10944.txt)

### Abstract (原文)
> Phishing is a persistent cybersecurity threat in today's digital landscape. This paper introduces Phishsense-1B, a refined version of the Llama-Guard-3-1B model, specifically tailored for phishing detection and reasoning. This adaptation utilizes Low-Rank Adaptation (LoRA) and the GuardReasoner finetuning methodology. We outline our LoRA-based fine-tuning process, describe the balanced dataset comprising phishing and benign emails, and highlight significant performance improvements over the original model. Our findings indicate that Phishsense-1B achieves an impressive 97.5% accuracy on a custom dataset and maintains strong performance with 70% accuracy on a challenging real-world dataset. This performance notably surpasses both unadapted models and BERT-based detectors. Additionally, we examine current state-of-the-art detection methods, compare prompt-engineering with fine-tuning strategies, and explore potential deployment scenarios.

---
type: "security-paper"
title: "LLMAC: A Global and Explainable Access Control Framework with Large Language Model"
title_ja: "LLMAC: A Global and Explainable Access Control Framework with Large Language Model（セキュリティ分析論文）"
description: "LLMAC: A Global and Explainable Access Control Framework with Large Language Model（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2602.09392v1"
tags:
  - "cs.AI"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
timestamp: "2026-08-14T06:11:07Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-02-10/2602.09392_meta.json"
  published_date: "2026-02-10T04:02:11Z"
  authors:
    - "Sharif Noor Zisad"
    - "Ragib Hasan"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# LLMAC: A Global and Explainable Access Control Framework with Large Language Model
### (日本語題名: LLMAC: A Global and Explainable Access Control Framework with Large Language Model（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2602.09392v1`](https://arxiv.org/abs/2602.09392v1) | Raw Meta = [`2602.09392_meta.json`](../../raw_data/2026-02-10/2602.09392_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「LLMAC: A Global and Explainable Access Control Framework with Large Language Model（セキュリティ分析論文）」（原題: LLMAC: A Global and Explainable Access Control Framework with Large Language Model / arXiv: 2602.09392v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2602.09392v1`
- **論文URL**: [https://arxiv.org/abs/2602.09392v1](https://arxiv.org/abs/2602.09392v1)
- **PDFリンク**: [https://arxiv.org/pdf/2602.09392v1.pdf](https://arxiv.org/pdf/2602.09392v1.pdf)
- **著者**: Sharif Noor Zisad, Ragib Hasan
- **公開日時**: `2026-02-10T04:02:11Z`
- **カテゴリ**: `cs.CR, cs.AI`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-02-10/2602.09392_meta.json) | [`PDF`](../../raw_data/2026-02-10/2602.09392.pdf) | [`TXT`](../../raw_data/2026-02-10/2602.09392.txt)

### Abstract (原文)
> Today's business organizations need access control systems that can handle complex, changing security requirements that go beyond what traditional methods can manage. Current approaches, such as Role-Based Access Control (RBAC), Attribute-Based Access Control (ABAC), and Discretionary Access Control (DAC), were designed for specific purposes. They cannot effectively manage the dynamic, situation-dependent workflows that modern systems require. In this research, we introduce LLMAC, a new unified approach using Large Language Models (LLMs) to combine these different access control methods into one comprehensive, understandable system. We used an extensive synthetic dataset that represents complex real-world scenarios, including policies for ownership verification, version management, workflow processes, and dynamic role separation. Using Mistral 7B, our trained LLM model achieved outstanding results with 98.5% accuracy, significantly outperforming traditional methods (RBAC: 14.5%, ABAC: 58.5%, DAC: 27.5%) while providing clear, human readable explanations for each decision. Performance testing shows that the system can be practically deployed with reasonable response times and computing resources.

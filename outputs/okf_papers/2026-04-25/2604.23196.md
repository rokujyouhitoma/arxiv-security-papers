---
type: "security-paper"
title: "AsmRAG: LLM-Driven Malware Detection by Retrieving Functionally Similar Assembly Code"
title_ja: "AsmRAG: LLM-Driven マルウェア検出 by Retrieving Functionally Similar Assembly Code"
description: "AsmRAG: LLM-Driven マルウェア検出 by Retrieving Functionally Similar Assembly Code — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.23196v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
timestamp: "2026-08-14T04:52:55Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-25/2604.23196_meta.json"
  published_date: "2026-04-25T08:05:50Z"
  authors:
    - "ElMouatez Billah Karbab"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# AsmRAG: LLM-Driven Malware Detection by Retrieving Functionally Similar Assembly Code
### (日本語題名: AsmRAG: LLM-Driven マルウェア検出 by Retrieving Functionally Similar Assembly Code)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.23196v1`](https://arxiv.org/abs/2604.23196v1) | Raw Meta = [`2604.23196_meta.json`](../../raw_data/2026-04-25/2604.23196_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「AsmRAG: LLM-Driven マルウェア検出 by Retrieving Functionally Similar Assembly Code」（原題: AsmRAG: LLM-Driven Malware Detection by Retrieving Functionally Similar Assembly Code / arXiv: 2604.23196v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: malware, security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: malware, security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.23196v1`
- **論文URL**: [https://arxiv.org/abs/2604.23196v1](https://arxiv.org/abs/2604.23196v1)
- **PDFリンク**: [https://arxiv.org/pdf/2604.23196v1.pdf](https://arxiv.org/pdf/2604.23196v1.pdf)
- **著者**: ElMouatez Billah Karbab
- **公開日時**: `2026-04-25T08:05:50Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-25/2604.23196_meta.json) | [`PDF`](../../raw_data/2026-04-25/2604.23196.pdf) | [`TXT`](../../raw_data/2026-04-25/2604.23196.txt)

### Abstract (原文)
> Deep learning malware detectors achieve high classification accuracy but suffer from severe interpretability limitations, typically returning probabilistic verdicts that lack forensic context. We introduce AsmRAG, a framework performing malware analysis through Assembly-Level Retrieval-Augmented Generation. Unlike classifiers built on global statistical features, AsmRAG reformulates detection as an evidence-based retrieval task. The system uses a code-specialized Large Language Model (LLM) to analyze assembly functions and convert them into semantic embeddings. This process constructs a searchable knowledge base resilient to syntactic obfuscation. For inference, we propose a Density-Weighted Anchor Selection mechanism that isolates the primary unit of malicious logic within a binary to extract verifiable forensic evidence and resist evasion attempts. Testing on a curated dataset of 40k binaries shows AsmRAG reaching a detection F1-score of 96% alongside a family attribution F1-score of 95%. Comparisons confirm this semantic retrieval approach remains robust against metamorphic obfuscation. When holistic baselines (EMBER and ResNeXt) degrade, our methodology gives Security Operations Centers a transparent and reliable alternative.

---
type: "security-paper"
title: "Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLM Agents in Capture the Flag Challenges"
title_ja: "Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLMエージェント in Capture the Flag Challenges"
description: "Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLMエージェント in Capture the Flag Challenges — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.19354v2"
tags:
  - "cs.AI"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.SE"
timestamp: "2026-08-14T06:11:06Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-21/2604.19354_meta.json"
  published_date: "2026-04-21T11:35:33Z"
  authors:
    - "Ali Al-Kaswan"
    - "Maksim Plotnikov"
    - "Maxim Hájek"
    - "Roland Vízner"
    - "Arie van Deursen"
    - "Maliheh Izadi"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLM Agents in Capture the Flag Challenges
### (日本語題名: Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLMエージェント in Capture the Flag Challenges)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.19354v2`](https://arxiv.org/abs/2604.19354v2) | Raw Meta = [`2604.19354_meta.json`](../../raw_data/2026-04-21/2604.19354_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLMエージェント in Capture the Flag Challenges」（原題: Do Agents Dream of Root Shells? Partial-Credit Evaluation of LLM Agents in Capture the Flag Challenges / arXiv: 2604.19354v2）は、cs.AI 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.19354v2`
- **論文URL**: [https://arxiv.org/abs/2604.19354v2](https://arxiv.org/abs/2604.19354v2)
- **PDFリンク**: [https://arxiv.org/pdf/2604.19354v2.pdf](https://arxiv.org/pdf/2604.19354v2.pdf)
- **著者**: Ali Al-Kaswan, Maksim Plotnikov, Maxim Hájek, Roland Vízner, Arie van Deursen, Maliheh Izadi
- **公開日時**: `2026-04-21T11:35:33Z`
- **カテゴリ**: `cs.AI, cs.CR, cs.SE`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-21/2604.19354_meta.json) | [`PDF`](../../raw_data/2026-04-21/2604.19354.pdf) | [`TXT`](../../raw_data/2026-04-21/2604.19354.txt)

### Abstract (原文)
> Large Language Model (LLM) agents are increasingly proposed for autonomous cybersecurity tasks, but their capabilities in realistic offensive settings remain poorly understood. We present DeepRed, an open-source benchmark for evaluating LLM-based agents on realistic Capture The Flag (CTF) challenges in isolated virtualized environments. DeepRed places an agent in a Kali attacker environment with terminal tools and optional web search, connected over a private network to a target challenge, and records full execution traces for analysis. To move beyond binary solved/unsolved outcomes, we introduce a partial-credit scoring method based on challenge-specific checkpoints derived from public writeups, together with an automated summarise-then-judge labelling pipeline for assigning checkpoint completion from logs. Using DeepRed, we benchmark ten commercially accessible LLMs on ten VM-based CTF challenges spanning different challenge categories. The results indicate that current agents remain limited: the best model achieves only 35% average checkpoint completion, performing strongest on common challenge types and weakest on tasks requiring non-standard discovery and longer-horizon adaptation.

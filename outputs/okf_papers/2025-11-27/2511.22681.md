---
type: "security-paper"
title: "CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs"
title_ja: "CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs（セキュリティ分析論文）"
description: "CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2511.22681v2"
tags:
  - "cybersecurity"
  - "executive-summary"
  - "cs.CR"
  - "arXiv"
timestamp: "2026-08-14T06:11:09Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-11-27/2511.22681_meta.json"
  published_date: "2025-11-27T18:30:19Z"
  authors:
    - "Mohaiminul Al Nahian"
    - "Abeer Matar A. Almalky"
    - "Gamana Aragonda"
    - "Ranyang Zhou"
    - "Sabbir Ahmed"
    - "Dmitry Ponomarev"
    - "Li Yang"
    - "Shaahin Angizi"
    - "Adnan Siraj Rakin"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs
### (日本語題名: CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2511.22681v2`](https://arxiv.org/abs/2511.22681v2) | Raw Meta = [`2511.22681_meta.json`](../../raw_data/2025-11-27/2511.22681_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs（セキュリティ分析論文）」（原題: CacheTrap: Unveiling a Stealthier Gray-Box Trojan against LLMs / arXiv: 2511.22681v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2511.22681v2`
- **論文URL**: [https://arxiv.org/abs/2511.22681v2](https://arxiv.org/abs/2511.22681v2)
- **PDFリンク**: [https://arxiv.org/pdf/2511.22681v2.pdf](https://arxiv.org/pdf/2511.22681v2.pdf)
- **著者**: Mohaiminul Al Nahian, Abeer Matar A. Almalky, Gamana Aragonda, Ranyang Zhou, Sabbir Ahmed, Dmitry Ponomarev, Li Yang, Shaahin Angizi, Adnan Siraj Rakin
- **公開日時**: `2025-11-27T18:30:19Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-11-27/2511.22681_meta.json) | [`PDF`](../../raw_data/2025-11-27/2511.22681.pdf) | [`TXT`](../../raw_data/2025-11-27/2511.22681.txt)

### Abstract (原文)
> The rapid advancement of large language models (LLMs) has sparked growing interest in understanding their security vulnerabilities, particularly Trojan attacks that enable stealthy manipulation of model behavior. Traditional Trojan methods typically alter inputs and/or model weights, relying on white-box assumptions that require access to data or model internal parameters. In this work, we present CacheTrap, the first gray-box Trojan attack targeting the Key-Value (KV) cache of LLMs. This method induces a single-bit flip in the KV cache, serving as a transient trigger. When activated, this trigger causes the model to exhibit targeted actions without changing inputs or model weights. CacheTrap introduces an efficient search algorithm to locate vulnerable positions in the KV cache, independent of model weights or datasets. Extensive experiments on five open-source LLMs show a remarkable 100% attack success rate (with the trigger) while preserving benign accuracy (without the trigger) by flipping just one bit in the KV cache.

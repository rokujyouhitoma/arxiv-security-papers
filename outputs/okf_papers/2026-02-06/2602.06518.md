---
type: "security-paper"
title: "Sequential Auditing for f-Differential Privacy"
title_ja: "Sequential Auditing for f-Differential Privacy（セキュリティ分析論文）"
description: "Sequential Auditing for f-Differential Privacy（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2602.06518v2"
tags:
  - "stat.ME"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "stat.ML"
timestamp: "2026-08-14T06:11:08Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-02-06/2602.06518_meta.json"
  published_date: "2026-02-06T09:22:24Z"
  authors:
    - "Tim Kutta"
    - "Martin Dunsche"
    - "Yu Wei"
    - "Vassilis Zikas"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Sequential Auditing for f-Differential Privacy
### (日本語題名: Sequential Auditing for f-Differential Privacy（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2602.06518v2`](https://arxiv.org/abs/2602.06518v2) | Raw Meta = [`2602.06518_meta.json`](../../raw_data/2026-02-06/2602.06518_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Sequential Auditing for f-Differential Privacy（セキュリティ分析論文）」（原題: Sequential Auditing for f-Differential Privacy / arXiv: 2602.06518v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: privacy)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: privacy)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2602.06518v2`
- **論文URL**: [https://arxiv.org/abs/2602.06518v2](https://arxiv.org/abs/2602.06518v2)
- **PDFリンク**: [https://arxiv.org/pdf/2602.06518v2.pdf](https://arxiv.org/pdf/2602.06518v2.pdf)
- **著者**: Tim Kutta, Martin Dunsche, Yu Wei, Vassilis Zikas
- **公開日時**: `2026-02-06T09:22:24Z`
- **カテゴリ**: `cs.CR, stat.ME, stat.ML`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-02-06/2602.06518_meta.json) | [`PDF`](../../raw_data/2026-02-06/2602.06518.pdf) | [`TXT`](../../raw_data/2026-02-06/2602.06518.txt)

### Abstract (原文)
> We present new auditors to assess Differential Privacy (DP) of an algorithm based on output samples. Such empirical auditors are common to check for algorithmic correctness and implementation bugs. Most existing auditors are batch-based or targeted toward the traditional notion of $(\varepsilon,δ)$-DP; typically both. In this work, we shift the focus to the highly expressive privacy concept of $f$-DP, in which the entire privacy behavior is captured by a single tradeoff curve. Our auditors detect violations across the full privacy spectrum with statistical significance guarantees, which are supported by theory and simulations. Most importantly, and in contrast to prior work, our auditors do not require a user-specified sample size as an input. Rather, they adaptively determine a near-optimal number of samples needed to reach a decision, thereby avoiding the excessively large sample sizes common in many auditing studies. This reduction in sampling cost becomes especially beneficial for expensive training procedures such as DP-SGD. Our method supports both whitebox and blackbox settings and can also be executed in one-run frameworks.

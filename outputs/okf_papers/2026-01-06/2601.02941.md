---
type: "security-paper"
title: "SastBench: A Benchmark for Testing Agentic SAST Triage"
title_ja: "SastBench: A Benchmark for Testing Agentic SAST Triage（セキュリティ分析論文）"
description: "SastBench: A Benchmark for Testing Agentic SAST Triage（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2601.02941v1"
tags:
  - "cs.AI"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "cs.CL"
  - "arXiv"
timestamp: "2026-08-14T06:11:08Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-01-06/2601.02941_meta.json"
  published_date: "2026-01-06T11:36:30Z"
  authors:
    - "Jake Feiglin"
    - "Guy Dar"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# SastBench: A Benchmark for Testing Agentic SAST Triage
### (日本語題名: SastBench: A Benchmark for Testing Agentic SAST Triage（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2601.02941v1`](https://arxiv.org/abs/2601.02941v1) | Raw Meta = [`2601.02941_meta.json`](../../raw_data/2026-01-06/2601.02941_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「SastBench: A Benchmark for Testing Agentic SAST Triage（セキュリティ分析論文）」（原題: SastBench: A Benchmark for Testing Agentic SAST Triage / arXiv: 2601.02941v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2601.02941v1`
- **論文URL**: [https://arxiv.org/abs/2601.02941v1](https://arxiv.org/abs/2601.02941v1)
- **PDFリンク**: [https://arxiv.org/pdf/2601.02941v1.pdf](https://arxiv.org/pdf/2601.02941v1.pdf)
- **著者**: Jake Feiglin, Guy Dar
- **公開日時**: `2026-01-06T11:36:30Z`
- **カテゴリ**: `cs.CR, cs.AI, cs.CL`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-01-06/2601.02941_meta.json) | [`PDF`](../../raw_data/2026-01-06/2601.02941.pdf) | [`TXT`](../../raw_data/2026-01-06/2601.02941.txt)

### Abstract (原文)
> SAST (Static Application Security Testing) tools are among the most widely used techniques in defensive cybersecurity, employed by commercial and non-commercial organizations to identify potential vulnerabilities in software. Despite their great utility, they generate numerous false positives, requiring costly manual filtering (aka triage). While LLM-powered agents show promise for automating cybersecurity tasks, existing benchmarks fail to emulate real-world SAST finding distributions. We introduce SastBench, a benchmark for evaluating SAST triage agents that combines real CVEs as true positives with filtered SAST tool findings as approximate false positives. SastBench features an agent-agnostic design. We evaluate different agents on the benchmark and present a comparative analysis of their performance, provide a detailed analysis of the dataset, and discuss the implications for future development.

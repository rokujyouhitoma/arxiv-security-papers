---
type: "security-paper"
title: "NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist"
title_ja: "NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist（セキュリティ分析論文）"
description: "NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2602.16756v1"
tags:
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.SE"
timestamp: "2026-08-14T06:11:07Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-02-18/2602.16756_meta.json"
  published_date: "2026-02-18T09:41:51Z"
  authors:
    - "Johannes Bertram"
    - "Jonas Geiping"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist
### (日本語題名: NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2602.16756v1`](https://arxiv.org/abs/2602.16756v1) | Raw Meta = [`2602.16756_meta.json`](../../raw_data/2026-02-18/2602.16756_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist（セキュリティ分析論文）」（原題: NESSiE: The Necessary Safety Benchmark -- Identifying Errors that should not Exist / arXiv: 2602.16756v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, risk, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, risk, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2602.16756v1`
- **論文URL**: [https://arxiv.org/abs/2602.16756v1](https://arxiv.org/abs/2602.16756v1)
- **PDFリンク**: [https://arxiv.org/pdf/2602.16756v1.pdf](https://arxiv.org/pdf/2602.16756v1.pdf)
- **著者**: Johannes Bertram, Jonas Geiping
- **公開日時**: `2026-02-18T09:41:51Z`
- **カテゴリ**: `cs.CR, cs.SE`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-02-18/2602.16756_meta.json) | [`PDF`](../../raw_data/2026-02-18/2602.16756.pdf) | [`TXT`](../../raw_data/2026-02-18/2602.16756.txt)

### Abstract (原文)
> We introduce NESSiE, the NEceSsary SafEty benchmark for large language models (LLMs). With minimal test cases of information and access security, NESSiE reveals safety-relevant failures that should not exist, given the low complexity of the tasks. NESSiE is intended as a lightweight, easy-to-use sanity check for language model safety and, as such, is not sufficient for guaranteeing safety in general -- but we argue that passing this test is necessary for any deployment. However, even state-of-the-art LLMs do not reach 100% on NESSiE and thus fail our necessary condition of language model safety, even in the absence of adversarial attacks. Our Safe & Helpful (SH) metric allows for direct comparison of the two requirements, showing models are biased toward being helpful rather than safe. We further find that disabled reasoning for some models, but especially a benign distraction context degrade model performance. Overall, our results underscore the critical risks of deploying such models as autonomous agents in the wild. We make the dataset, package and plotting code publicly available.

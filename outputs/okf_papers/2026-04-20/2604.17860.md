---
type: "security-paper"
title: "TitanCA: Lessons from Orchestrating LLM Agents to Discover 100+ CVEs"
title_ja: "TitanCA: Lessons from Orchestrating LLMエージェント to Discover 100+ CVEs"
description: "TitanCA: Lessons from Orchestrating LLMエージェント to Discover 100+ CVEs — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.17860v2"
tags:
  - "cybersecurity"
  - "executive-summary"
  - "cs.CR"
  - "arXiv"
timestamp: "2026-08-14T06:11:06Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-20/2604.17860_meta.json"
  published_date: "2026-04-20T06:12:51Z"
  authors:
    - "Ting Zhang"
    - "Yikun Li"
    - "Chengran Yang"
    - "Ratnadira Widyasari"
    - "Yue Liu"
    - "Ngoc Tan Bui"
    - "Phuc Thanh Nguyen"
    - "Yan Naing Tun"
    - "Ivana Clairine Irsan"
    - "Huu Hung Nguyen"
    - "Huihui Huang"
    - "Jinfeng Jiang"
    - "Lwin Khin Shar"
    - "Eng Lieh Ouh"
    - "David Lo"
    - "Hong Jin Kang"
    - "Yide Yin"
    - "Wen Bin Leow"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# TitanCA: Lessons from Orchestrating LLM Agents to Discover 100+ CVEs
### (日本語題名: TitanCA: Lessons from Orchestrating LLMエージェント to Discover 100+ CVEs)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.17860v2`](https://arxiv.org/abs/2604.17860v2) | Raw Meta = [`2604.17860_meta.json`](../../raw_data/2026-04-20/2604.17860_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「TitanCA: Lessons from Orchestrating LLMエージェント to Discover 100+ CVEs」（原題: TitanCA: Lessons from Orchestrating LLM Agents to Discover 100+ CVEs / arXiv: 2604.17860v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, threat, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, threat, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.17860v2`
- **論文URL**: [https://arxiv.org/abs/2604.17860v2](https://arxiv.org/abs/2604.17860v2)
- **PDFリンク**: [https://arxiv.org/pdf/2604.17860v2.pdf](https://arxiv.org/pdf/2604.17860v2.pdf)
- **著者**: Ting Zhang, Yikun Li, Chengran Yang, Ratnadira Widyasari, Yue Liu, Ngoc Tan Bui, Phuc Thanh Nguyen, Yan Naing Tun, Ivana Clairine Irsan, Huu Hung Nguyen, Huihui Huang, Jinfeng Jiang, Lwin Khin Shar, Eng Lieh Ouh, David Lo, Hong Jin Kang, Yide Yin, Wen Bin Leow
- **公開日時**: `2026-04-20T06:12:51Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-20/2604.17860_meta.json) | [`PDF`](../../raw_data/2026-04-20/2604.17860.pdf) | [`TXT`](../../raw_data/2026-04-20/2604.17860.txt)

### Abstract (原文)
> Software vulnerabilities remain one of the most persistent threats to modern digital infrastructure. While static application security testing (SAST) tools have long served as the first line of defense, they suffer from high false-positive rates. This article presents TitanCA, a collaborative project between Singapore Management University and GovTech Singapore that orchestrates multiple large language model (LLM)-powered agents into a unified vulnerability discovery pipeline. Applied in open-source software, TitanCA has discovered 203 confirmed zero-day vulnerabilities and yielded 118 CVEs. We describe the four-module architecture, i.e., matching, filtering, inspection, and adaptation, and share key lessons from building and deploying an LLM-based vulnerability discovery solution in practice.

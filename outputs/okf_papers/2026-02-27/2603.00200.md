---
type: "security-paper"
title: "LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance"
title_ja: "LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance（セキュリティ分析論文）"
description: "LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2603.00200v1"
tags:
  - "cs.AI"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
timestamp: "2026-08-14T06:11:07Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-02-27/2603.00200_meta.json"
  published_date: "2026-02-27T08:42:52Z"
  authors:
    - "Chuanming Tang"
    - "Ling Qing"
    - "Shifeng Chen"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance
### (日本語題名: LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2603.00200v1`](https://arxiv.org/abs/2603.00200v1) | Raw Meta = [`2603.00200_meta.json`](../../raw_data/2026-02-27/2603.00200_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance（セキュリティ分析論文）」（原題: LiaisonAgent: An Multi-Agent Framework for Autonomous Risk Investigation and Governance / arXiv: 2603.00200v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, risk, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, risk, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2603.00200v1`
- **論文URL**: [https://arxiv.org/abs/2603.00200v1](https://arxiv.org/abs/2603.00200v1)
- **PDFリンク**: [https://arxiv.org/pdf/2603.00200v1.pdf](https://arxiv.org/pdf/2603.00200v1.pdf)
- **著者**: Chuanming Tang, Ling Qing, Shifeng Chen
- **公開日時**: `2026-02-27T08:42:52Z`
- **カテゴリ**: `cs.CR, cs.AI`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-02-27/2603.00200_meta.json) | [`PDF`](../../raw_data/2026-02-27/2603.00200.pdf) | [`TXT`](../../raw_data/2026-02-27/2603.00200.txt)

### Abstract (原文)
> The rapid evolution of sophisticated cyberattacks has strained modern Security Operations Centers (SOC), which traditionally rely on rule-based or signature-driven detection systems. These legacy frameworks often generate high volumes of technical alerts that lack organizational context, leading to analyst fatigue and delayed incident responses. This paper presents LiaisonAgent, an autonomous multi-agent system designed to bridge the gap between technical risk detection and business-level risk governance. Built upon the QWQ-32B large reasoning model, LiaisonAgent integrates specialized sub-agents, including human-computer interaction agents, comprehensive judgment agents, and automated disposal agents-to execute end-to-end investigation workflows. The system leverages a hybrid planning architecture that combines deterministic workflows for compliance with autonomous reasoning based on the ReAct paradigm to handle ambiguous operational scenarios. Experimental evaluations across diverse security contexts, such as large-scale data exfiltration and unauthorized account borrowing, achieve an end-to-end tool-calling success rate of 97.8% and a risk judgment accuracy of 95%. Furthermore, the system exhibits significant resilience against out-of-distribution noise and adversarial prompt injections, while achieving a 92.7% reduction in manual investigation overhead.

---
type: "security-paper"
title: "FedBAP: Backdoor Defense via Benign Adversarial Perturbation in Federated Learning"
title_ja: "FedBAP: Backdoor Defense via Benign Adversarial Perturbation in 連合学習"
description: "FedBAP: Backdoor Defense via Benign Adversarial Perturbation in 連合学習 — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2507.21177v1"
tags:
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
  - "cybersecurity"
  - "executive-summary"
timestamp: "2026-08-14T07:15:31Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-07-26/2507.21177_meta.json"
  published_date: "2025-07-26T07:00:32Z"
  authors:
    - "Xinhai Yan"
    - "Libing Wu"
    - "Zhuangzhuang Zhang"
    - "Bingyi Liu"
    - "Lijuan Huo"
    - "Jing Wang"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# FedBAP: Backdoor Defense via Benign Adversarial Perturbation in Federated Learning
### (日本語題名: FedBAP: Backdoor Defense via Benign Adversarial Perturbation in 連合学習)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2507.21177v1`](https://arxiv.org/abs/2507.21177v1) | Raw Meta = [`2507.21177_meta.json`](../../raw_data/2025-07-26/2507.21177_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「FedBAP: Backdoor Defense via Benign Adversarial Perturbation in 連合学習」（原題: FedBAP: Backdoor Defense via Benign Adversarial Perturbation in Federated Learning / arXiv: 2507.21177v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, privacy)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, privacy)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2507.21177v1`
- **論文URL**: [https://arxiv.org/abs/2507.21177v1](https://arxiv.org/abs/2507.21177v1)
- **PDFリンク**: [https://arxiv.org/pdf/2507.21177v1.pdf](https://arxiv.org/pdf/2507.21177v1.pdf)
- **著者**: Xinhai Yan, Libing Wu, Zhuangzhuang Zhang, Bingyi Liu, Lijuan Huo, Jing Wang
- **公開日時**: `2025-07-26T07:00:32Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-07-26/2507.21177_meta.json) | [`PDF`](../../raw_data/2025-07-26/2507.21177.pdf) | [`TXT`](../../raw_data/2025-07-26/2507.21177.txt)

### Abstract (原文)
> Federated Learning (FL) enables collaborative model training while preserving data privacy, but it is highly vulnerable to backdoor attacks. Most existing defense methods in FL have limited effectiveness due to their neglect of the model's over-reliance on backdoor triggers, particularly as the proportion of malicious clients increases. In this paper, we propose FedBAP, a novel defense framework for mitigating backdoor attacks in FL by reducing the model's reliance on backdoor triggers. Specifically, first, we propose a perturbed trigger generation mechanism that creates perturbation triggers precisely matching backdoor triggers in location and size, ensuring strong influence on model outputs. Second, we utilize these perturbation triggers to generate benign adversarial perturbations that disrupt the model's dependence on backdoor triggers while forcing it to learn more robust decision boundaries. Finally, we design an adaptive scaling mechanism to dynamically adjust perturbation intensity, effectively balancing defense strength and model performance. The experimental results demonstrate that FedBAP reduces the attack success rates by 0.22%-5.34%, 0.48%-6.34%, and 97.22%-97.6% under three types of backdoor attacks, respectively. In particular, FedBAP demonstrates outstanding performance against novel backdoor attacks.

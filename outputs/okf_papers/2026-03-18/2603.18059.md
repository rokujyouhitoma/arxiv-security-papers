---
type: "security-paper"
title: "Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows"
title_ja: "Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows（セキュリティ分析論文）"
description: "Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2603.18059v1"
tags:
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.SE"
timestamp: "2026-08-14T06:11:07Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-03-18/2603.18059_meta.json"
  published_date: "2026-03-18T01:19:33Z"
  authors:
    - "Akshey Sigdel"
    - "Rista Baral"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows
### (日本語題名: Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2603.18059v1`](https://arxiv.org/abs/2603.18059v1) | Raw Meta = [`2603.18059_meta.json`](../../raw_data/2026-03-18/2603.18059_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows（セキュリティ分析論文）」（原題: Guardrails as Infrastructure: Policy-First Control for Tool-Orchestrated Workflows / arXiv: 2603.18059v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: risk, leak, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: risk, leak, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2603.18059v1`
- **論文URL**: [https://arxiv.org/abs/2603.18059v1](https://arxiv.org/abs/2603.18059v1)
- **PDFリンク**: [https://arxiv.org/pdf/2603.18059v1.pdf](https://arxiv.org/pdf/2603.18059v1.pdf)
- **著者**: Akshey Sigdel, Rista Baral
- **公開日時**: `2026-03-18T01:19:33Z`
- **カテゴリ**: `cs.CR, cs.SE`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-03-18/2603.18059_meta.json) | [`PDF`](../../raw_data/2026-03-18/2603.18059.pdf) | [`TXT`](../../raw_data/2026-03-18/2603.18059.txt)

### Abstract (原文)
> Tool-using automation systems, from scripts and CI bots to agentic assistants, fail in recurring patterns. Common failures include unsafe side effects, invalid arguments, uncontrolled retries, and leakage of sensitive outputs. Many mitigations are model-centric and prompt-dependent, so they are brittle and do not generalize to non-LLM callers. We present Policy-First Tooling, a model-agnostic permission layer that mediates tool invocation through explicit constraints, risk-aware gating, recovery controls, and auditable explanations. The paper contributes a compact policy DSL, a runtime enforcement architecture with actionable rationale and fix hints, and a reproducible benchmark based on trace replay with controlled fault and misuse injection. In 225 controlled runs across five policy packs and three fault profiles, stricter packs improve violation prevention from 0.000 in P0 to 0.681 in P4, while task success drops from 0.356 to 0.067. Retry amplification decreases from 3.774 in P0 to 1.378 in P4, and leakage recall reaches 0.875 under injected secret outputs. These results make safety to utility trade-offs explicit and measurable.

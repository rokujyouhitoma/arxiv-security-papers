---
type: "security-paper"
title: "FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis"
title_ja: "FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis（セキュリティ分析論文）"
description: "FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2605.09664v2"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.LG"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:54Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-05-10/2605.09664_meta.json"
  published_date: "2026-05-10T17:30:11Z"
  authors:
    - "Zahra Asadi"
    - "Haeseung Jeon"
    - "Sohyun Han"
    - "Md Mahmuduzzaman Kamol"
    - "Se Eun Oh"
    - "Mohammad Saidur Rahman"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis
### (日本語題名: FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2605.09664v2`](https://arxiv.org/abs/2605.09664v2) | Raw Meta = [`2605.09664_meta.json`](../../raw_data/2026-05-10/2605.09664_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis（セキュリティ分析論文）」（原題: FreeMOCA: Memory-Free Continual Learning for Malicious Code Analysis / arXiv: 2605.09664v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: threat, exploit, malware)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: threat, exploit, malware)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2605.09664v2`
- **論文URL**: [https://arxiv.org/abs/2605.09664v2](https://arxiv.org/abs/2605.09664v2)
- **PDFリンク**: [https://arxiv.org/pdf/2605.09664v2.pdf](https://arxiv.org/pdf/2605.09664v2.pdf)
- **著者**: Zahra Asadi, Haeseung Jeon, Sohyun Han, Md Mahmuduzzaman Kamol, Se Eun Oh, Mohammad Saidur Rahman
- **公開日時**: `2026-05-10T17:30:11Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-05-10/2605.09664_meta.json) | [`PDF`](../../raw_data/2026-05-10/2605.09664.pdf) | [`TXT`](../../raw_data/2026-05-10/2605.09664.txt)

### Abstract (原文)
> As over 200 million new malware samples are identified each year, antivirus systems must continuously adapt to the evolving threat landscape. However, retraining solely on new samples leads to catastrophic forgetting and exploitable blind spots, while retraining on the entire dataset incurs substantial computational cost. We propose FreeMOCA, a memory- and compute-efficient continual learning framework for malicious code analysis that preserves prior knowledge via adaptive layer-wise interpolation between consecutive task updates, leveraging the fact that warm-started task optima are connected by low-loss paths in parameter space. We evaluate FreeMOCA in both class-incremental (Class-IL) and domain-incremental (Domain-IL) settings on large-scale Windows (EMBER) and Android (AZ) malware benchmarks. FreeMOCA achieves substantial gains in Class-IL, outperforming 11 baselines on both EMBER and AZ benchmarks. It also significantly reduces forgetting, achieving the best retention across baselines, and improving accuracy by up to 42% and 37% on EMBER and AZ, respectively. These results demonstrate that warm-started interpolation in parameter space provides a scalable and effective alternative to replay for continual malware detection. Code is available at: https://github.com/IQSeC-Lab/FreeMOCA.

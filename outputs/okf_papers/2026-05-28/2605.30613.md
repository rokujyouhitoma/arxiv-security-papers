---
type: "security-paper"
title: "CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs"
title_ja: "CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs（セキュリティ分析論文）"
description: "CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2605.30613v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.LG"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:53Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-05-28/2605.30613_meta.json"
  published_date: "2026-05-28T22:06:54Z"
  authors:
    - "Ryan Fahey"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs
### (日本語題名: CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2605.30613v1`](https://arxiv.org/abs/2605.30613v1) | Raw Meta = [`2605.30613_meta.json`](../../raw_data/2026-05-28/2605.30613_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs（セキュリティ分析論文）」（原題: CacheProbe: Auditing Prompt Cache Isolation in Gateway APIs / arXiv: 2605.30613v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, leak, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, leak, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2605.30613v1`
- **論文URL**: [https://arxiv.org/abs/2605.30613v1](https://arxiv.org/abs/2605.30613v1)
- **PDFリンク**: [https://arxiv.org/pdf/2605.30613v1.pdf](https://arxiv.org/pdf/2605.30613v1.pdf)
- **著者**: Ryan Fahey
- **公開日時**: `2026-05-28T22:06:54Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-05-28/2605.30613_meta.json) | [`PDF`](../../raw_data/2026-05-28/2605.30613.pdf) | [`TXT`](../../raw_data/2026-05-28/2605.30613.txt)

### Abstract (原文)
> Over the past year, prompt caching in Large Language Models (LLMs) has become increasingly more popular across inference APIs. Prompt caching helps save precious compute resources and speeds up response times by reusing parts of the KV cache of a specific prompt for another request. However, many implementations of prompt caching are not secure against timing attacks or even basic metadata disclosure. Gu et al. (ICML 2025) develop a method to audit prompt caching in LLMs. This paper investigates whether OpenRouter's API gateway architecture introduces prompt caching vulnerabilities that bypass provider-level prompt cache isolation guarantees. Most LLM inference providers implement per-account or per-organization prompt caching to prevent data leaks, but does routing through OpenRouter with shared organizational credentials inadvertently create global cache sharing across all OpenRouter users?

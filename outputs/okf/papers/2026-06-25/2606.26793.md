---
type: "security-paper"
title: "MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG"
title_ja: "MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG（セキュリティ分析論文）"
description: "MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2606.26793v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.AI"
  - "cs.LG"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:54Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-06-25/2606.26793_meta.json"
  published_date: "2026-06-25T09:26:49Z"
  authors:
    - "Inderjeet Singh"
    - "Andrés Murillo"
    - "Motoyoshi Sekiya"
    - "Yuki Unno"
    - "Junichi Suga"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG
### (日本語題名: MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2606.26793v1`](https://arxiv.org/abs/2606.26793v1) | Raw Meta = [`2606.26793_meta.json`](../../raw_data/2026-06-25/2606.26793_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG（セキュリティ分析論文）」（原題: MIRROR: Novelty-Constrained Memory-Guided MCTS Red-Teaming for Agentic RAG / arXiv: 2606.26793v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2606.26793v1`
- **論文URL**: [https://arxiv.org/abs/2606.26793v1](https://arxiv.org/abs/2606.26793v1)
- **PDFリンク**: [https://arxiv.org/pdf/2606.26793v1.pdf](https://arxiv.org/pdf/2606.26793v1.pdf)
- **著者**: Inderjeet Singh, Andrés Murillo, Motoyoshi Sekiya, Yuki Unno, Junichi Suga
- **公開日時**: `2026-06-25T09:26:49Z`
- **カテゴリ**: `cs.CR, cs.AI, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-06-25/2606.26793_meta.json) | [`PDF`](../../raw_data/2026-06-25/2606.26793.pdf) | [`TXT`](../../raw_data/2026-06-25/2606.26793.txt)

### Abstract (原文)
> Multimodal agentic retrieval-augmented generation (RAG) systems expand the attack surface beyond prompt injection to include text poisoning, image injection, direct-query attacks, and orchestrator-level tool manipulation. Existing red-teaming approaches are typically surface-specific and often recycle known attack templates; on text-poisoning benchmarks we measure 73-84% exact duplication. We present MIRROR, a unified cross-surface framework that performs memory-guided Monte Carlo tree search while conditioning candidate generation on retrieved context under an explicit novelty constraint. A deterministic Novelty Gate rejects any candidate matching the retrieval set under normalized comparison, allowing retrieval to inform search priors without enabling prompt copying. Across four attack surfaces on a multimodal agentic RAG target, MIRROR attains 76% ASR on image poisoning compared with 52% for baselines, 97% ASR on orchestrator attacks at half the query cost, and the lowest cross-surface variance (coefficient of variation 0.47). In contrast, specialized baselines collapse across surfaces: suffix optimization reaches 79% ASR on text poisoning but 1% on direct queries. We release ART-SafeBench with 41,815 in-package records and runtime adapters yielding 41,991+ total records across four surfaces.

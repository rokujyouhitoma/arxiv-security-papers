---
type: "security-paper"
title: "Tilikum: Transaction Fair Ordering on a DAG without Weak Edges"
title_ja: "Tilikum: Transaction Fair Ordering on a DAG without Weak Edges（セキュリティ分析論文）"
description: "Tilikum: Transaction Fair Ordering on a DAG without Weak Edges（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2606.27250v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.DC"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:54Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-06-25/2606.27250_meta.json"
  published_date: "2026-06-25T16:34:44Z"
  authors:
    - "Giulio Segalini"
    - "Yigit Çolakoğlu"
    - "Marko Putnik"
    - "Jérémie Decouchant"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Tilikum: Transaction Fair Ordering on a DAG without Weak Edges
### (日本語題名: Tilikum: Transaction Fair Ordering on a DAG without Weak Edges（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2606.27250v1`](https://arxiv.org/abs/2606.27250v1) | Raw Meta = [`2606.27250_meta.json`](../../raw_data/2026-06-25/2606.27250_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Tilikum: Transaction Fair Ordering on a DAG without Weak Edges（セキュリティ分析論文）」（原題: Tilikum: Transaction Fair Ordering on a DAG without Weak Edges / arXiv: 2606.27250v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2606.27250v1`
- **論文URL**: [https://arxiv.org/abs/2606.27250v1](https://arxiv.org/abs/2606.27250v1)
- **PDFリンク**: [https://arxiv.org/pdf/2606.27250v1.pdf](https://arxiv.org/pdf/2606.27250v1.pdf)
- **著者**: Giulio Segalini, Yigit Çolakoğlu, Marko Putnik, Jérémie Decouchant
- **公開日時**: `2026-06-25T16:34:44Z`
- **カテゴリ**: `cs.CR, cs.DC`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-06-25/2606.27250_meta.json) | [`PDF`](../../raw_data/2026-06-25/2606.27250.pdf) | [`TXT`](../../raw_data/2026-06-25/2606.27250.txt)

### Abstract (原文)
> Decentralized Finance (DeFi) applications rely heavily on the order in which transactions are executed, making them susceptible to reordering attacks that enable adversaries to extract Blockchain Extractable Value (BEV). While linear blockchain systems such as Ethereum have inspired extensive research into fair ordering mechanisms, DAG-based consensus protocols have remained largely unprotected despite their growing adoption for scalability and performance. In this paper, we introduce Tilikum, a DAG-based ledger protocol that ensures fair transaction ordering without relying on weak edges. Tilikum achieves ordering linearizability by leveraging median-based timestamp aggregation, or batch order fairness, while maintaining low data redundancy and robust garbage collection. We implemented Tilikum in Rust and evaluated it against representative baselines, namely Narwhal/Tusk, Pompē, Themis and FairDAG. Our results show that Tilikum achieves up to $39\times$ higher throughput than other fair-ordering baselines, while fully blocking state-of-the-art DAG-specific reordering attacks.

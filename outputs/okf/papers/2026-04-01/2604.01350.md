---
type: "security-paper"
title: "No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLM Agents"
title_ja: "No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLMエージェント"
description: "No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLMエージェント — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.01350v1"
tags:
  - "cs.AI"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "cs.CL"
  - "arXiv"
timestamp: "2026-08-14T06:11:06Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-01/2604.01350_meta.json"
  published_date: "2026-04-01T20:03:56Z"
  authors:
    - "Tiankai Yang"
    - "Jiate Li"
    - "Yi Nian"
    - "Shen Dong"
    - "Ruiyao Xu"
    - "Ryan Rossi"
    - "Kaize Ding"
    - "Yue Zhao"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLM Agents
### (日本語題名: No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLMエージェント)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.01350v1`](https://arxiv.org/abs/2604.01350v1) | Raw Meta = [`2604.01350_meta.json`](../../raw_data/2026-04-01/2604.01350_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLMエージェント」（原題: No Attacker Needed: Unintentional Cross-User Contamination in Shared-State LLM Agents / arXiv: 2604.01350v1）は、cs.CL 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, risk, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, risk, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.01350v1`
- **論文URL**: [https://arxiv.org/abs/2604.01350v1](https://arxiv.org/abs/2604.01350v1)
- **PDFリンク**: [https://arxiv.org/pdf/2604.01350v1.pdf](https://arxiv.org/pdf/2604.01350v1.pdf)
- **著者**: Tiankai Yang, Jiate Li, Yi Nian, Shen Dong, Ruiyao Xu, Ryan Rossi, Kaize Ding, Yue Zhao
- **公開日時**: `2026-04-01T20:03:56Z`
- **カテゴリ**: `cs.CL, cs.AI, cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-01/2604.01350_meta.json) | [`PDF`](../../raw_data/2026-04-01/2604.01350.pdf) | [`TXT`](../../raw_data/2026-04-01/2604.01350.txt)

### Abstract (原文)
> LLM-based agents increasingly operate across repeated sessions, maintaining task states to ensure continuity. In many deployments, a single agent serves multiple users within a team or organization, reusing a shared knowledge layer across user identities. This shared persistence expands the failure surface: information that is locally valid for one user can silently degrade another user's outcome when the agent reapplies it without regard for scope. We refer to this failure mode as unintentional cross-user contamination (UCC). Unlike adversarial memory poisoning, UCC requires no attacker; it arises from benign interactions whose scope-bound artifacts persist and are later misapplied. We formalize UCC through a controlled evaluation protocol, introduce a taxonomy of three contamination types, and evaluate the problem in two shared-state mechanisms. Under raw shared state, benign interactions alone produce contamination rates of 57--71%. A write-time sanitization is effective when shared state is conversational, but leaves substantial residual risk when shared state includes executable artifacts, with contamination often manifesting as silent wrong answers. These results indicate that shared-state agents need artifact-level defenses beyond text-level sanitization to prevent silent cross-user failures.

---
type: "security-paper"
title: "Jailbreak Attack Initializations as Extractors of Compliance Directions"
title_ja: "Jailbreak Attack Initializations as Extractors of Compliance Directions（セキュリティ分析論文）"
description: "Jailbreak Attack Initializations as Extractors of Compliance Directions（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2502.09755v4"
tags:
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
  - "cybersecurity"
  - "executive-summary"
timestamp: "2026-08-14T07:15:33Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-02-13/2502.09755_meta.json"
  published_date: "2025-02-13T20:25:40Z"
  authors:
    - "Amit Levi"
    - "Rom Himelstein"
    - "Yaniv Nemcovsky"
    - "Avi Mendelson"
    - "Chaim Baskin"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Jailbreak Attack Initializations as Extractors of Compliance Directions
### (日本語題名: Jailbreak Attack Initializations as Extractors of Compliance Directions（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2502.09755v4`](https://arxiv.org/abs/2502.09755v4) | Raw Meta = [`2502.09755_meta.json`](../../raw_data/2025-02-13/2502.09755_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Jailbreak Attack Initializations as Extractors of Compliance Directions（セキュリティ分析論文）」（原題: Jailbreak Attack Initializations as Extractors of Compliance Directions / arXiv: 2502.09755v4）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2502.09755v4`
- **論文URL**: [https://arxiv.org/abs/2502.09755v4](https://arxiv.org/abs/2502.09755v4)
- **PDFリンク**: [https://arxiv.org/pdf/2502.09755v4.pdf](https://arxiv.org/pdf/2502.09755v4.pdf)
- **著者**: Amit Levi, Rom Himelstein, Yaniv Nemcovsky, Avi Mendelson, Chaim Baskin
- **公開日時**: `2025-02-13T20:25:40Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-02-13/2502.09755_meta.json) | [`PDF`](../../raw_data/2025-02-13/2502.09755.pdf) | [`TXT`](../../raw_data/2025-02-13/2502.09755.txt)

### Abstract (原文)
> Safety-aligned LLMs respond to prompts with either compliance or refusal, each corresponding to distinct directions in the model's activation space. Recent works show that initializing attacks via self-transfer from other prompts significantly enhances their performance. However, the underlying mechanisms of these initializations remain unclear, and attacks utilize arbitrary or hand-picked initializations. This work presents that each gradient-based jailbreak attack and subsequent initialization gradually converge to a single compliance direction that suppresses refusal, thereby enabling an efficient transition from refusal to compliance. Based on this insight, we propose CRI, an initialization framework that aims to project unseen prompts further along compliance directions. We demonstrate our approach on multiple attacks, models, and datasets, achieving an increased attack success rate (ASR) and reduced computational overhead, highlighting the fragility of safety-aligned LLMs. A reference implementation is available at: https://amit1221levi.github.io/CRI-Jailbreak-Init-LLMs-evaluation.

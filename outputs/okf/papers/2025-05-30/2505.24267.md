---
type: "security-paper"
title: "MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection"
title_ja: "MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection（セキュリティ分析論文）"
description: "MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2505.24267v1"
tags:
  - "cs.CR"
  - "arXiv"
  - "cybersecurity"
  - "executive-summary"
timestamp: "2026-08-14T07:15:31Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-05-30/2505.24267_meta.json"
  published_date: "2025-05-30T06:45:31Z"
  authors:
    - "Liancheng Fang"
    - "Aiwei Liu"
    - "Henry Peng Zou"
    - "Yankai Chen"
    - "Hengrui Zhang"
    - "Zhongfen Deng"
    - "Philip S. Yu"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection
### (日本語題名: MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2505.24267v1`](https://arxiv.org/abs/2505.24267v1) | Raw Meta = [`2505.24267_meta.json`](../../raw_data/2025-05-30/2505.24267_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection（セキュリティ分析論文）」（原題: MUSE: Model-Agnostic Tabular Watermarking via Multi-Sample Selection / arXiv: 2505.24267v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2505.24267v1`
- **論文URL**: [https://arxiv.org/abs/2505.24267v1](https://arxiv.org/abs/2505.24267v1)
- **PDFリンク**: [https://arxiv.org/pdf/2505.24267v1.pdf](https://arxiv.org/pdf/2505.24267v1.pdf)
- **著者**: Liancheng Fang, Aiwei Liu, Henry Peng Zou, Yankai Chen, Hengrui Zhang, Zhongfen Deng, Philip S. Yu
- **公開日時**: `2025-05-30T06:45:31Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-05-30/2505.24267_meta.json) | [`PDF`](../../raw_data/2025-05-30/2505.24267.pdf) | [`TXT`](../../raw_data/2025-05-30/2505.24267.txt)

### Abstract (原文)
> We introduce MUSE, a watermarking algorithm for tabular generative models. Previous approaches typically leverage DDIM invertibility to watermark tabular diffusion models, but tabular diffusion models exhibit significantly poorer invertibility compared to other modalities, compromising performance. Simultaneously, tabular diffusion models require substantially less computation than other modalities, enabling a multi-sample selection approach to tabular generative model watermarking. MUSE embeds watermarks by generating multiple candidate samples and selecting one based on a specialized scoring function, without relying on model invertibility. Our theoretical analysis establishes the relationship between watermark detectability, candidate count, and dataset size, allowing precise calibration of watermarking strength. Extensive experiments demonstrate that MUSE achieves state-of-the-art watermark detectability and robustness against various attacks while maintaining data quality, and remains compatible with any tabular generative model supporting repeated sampling, effectively addressing key challenges in tabular data watermarking. Specifically, it reduces the distortion rates on fidelity metrics by 81-89%, while achieving a 1.0 TPR@0.1%FPR detection rate. Implementation of MUSE can be found at https://github.com/fangliancheng/MUSE.

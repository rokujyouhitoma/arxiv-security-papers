---
type: "security-paper"
title: "How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test"
title_ja: "How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test（セキュリティ分析論文）"
description: "How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2601.07084v1"
tags:
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.SE"
timestamp: "2026-08-14T06:11:08Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-01-11/2601.07084_meta.json"
  published_date: "2026-01-11T22:28:21Z"
  authors:
    - "Melissa Tessa"
    - "Iyiola E. Olatunji"
    - "Aicha War"
    - "Jacques Klein"
    - "Tegawendé F. Bissyandé"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test
### (日本語題名: How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2601.07084v1`](https://arxiv.org/abs/2601.07084v1) | Raw Meta = [`2601.07084_meta.json`](../../raw_data/2026-01-11/2601.07084_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test（セキュリティ分析論文）」（原題: How Secure is Secure Code Generation? Adversarial Prompts Put LLM Defenses to the Test / arXiv: 2601.07084v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, exploit, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, exploit, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2601.07084v1`
- **論文URL**: [https://arxiv.org/abs/2601.07084v1](https://arxiv.org/abs/2601.07084v1)
- **PDFリンク**: [https://arxiv.org/pdf/2601.07084v1.pdf](https://arxiv.org/pdf/2601.07084v1.pdf)
- **著者**: Melissa Tessa, Iyiola E. Olatunji, Aicha War, Jacques Klein, Tegawendé F. Bissyandé
- **公開日時**: `2026-01-11T22:28:21Z`
- **カテゴリ**: `cs.CR, cs.SE`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-01-11/2601.07084_meta.json) | [`PDF`](../../raw_data/2026-01-11/2601.07084.pdf) | [`TXT`](../../raw_data/2026-01-11/2601.07084.txt)

### Abstract (原文)
> Recent secure code generation methods, using vulnerability-aware fine-tuning, prefix-tuning, and prompt optimization, claim to prevent LLMs from producing insecure code. However, their robustness under adversarial conditions remains untested, and current evaluations decouple security from functionality, potentially inflating reported gains. We present the first systematic adversarial audit of state-of-the-art secure code generation methods (SVEN, SafeCoder, PromSec). We subject them to realistic prompt perturbations such as paraphrasing, cue inversion, and context manipulation that developers might inadvertently introduce or adversaries deliberately exploit. To enable fair comparison, we evaluate all methods under consistent conditions, jointly assessing security and functionality using multiple analyzers and executable tests. Our findings reveal critical robustness gaps: static analyzers overestimate security by 7 to 21 times, with 37 to 60% of ``secure'' outputs being non-functional. Under adversarial conditions, true secure-and-functional rates collapse to 3 to 17%. Based on these findings, we propose best practices for building and evaluating robust secure code generation methods. Our code is available.

...
description: "【提案】Gao, Feng, and Hu (GFH) subsequently introduced DGS on random prime-index superlattices...。実証評価によりAs a result, we obtain a ..."
...
本論文「Discrete Gaussian Sampling Meets BDGL Decoding: Solving the Shortest Vector Problem in $2^{0.5596n+o(n)}$ Time（セキュリティ分析論文）」（原題: Discrete Gaussian Sampling Meets BDGL Decoding: Solving the Shortest Vector Problem in $2^{0.5596n+o(n)}$ Time / IACR: 2026-1844）は、cryptography 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 【提案】Gao, Feng, and Hu (GFH) subsequently introduced DGS on random prime-index superlattices...。実証評価によりAs a result, we obtain a ...

### 2. 背景とセキュリティ上の課題 (Background & Problem)
【課題・脅威】Aggarwal, Dadush, Regev, and Stephens-Davidowitz (ADRS) gave a $2^{n+o(n)}$ time algori...
(主要キーワード: Shortest Vector Problem, Discrete Gaussian Sampling Meets, Time Aggarwal, prime-index superlattices)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
【提案技術】Gao, Feng, and Hu (GFH) subsequently introduced DGS on random prime-index superlattices...

### 4. セキュリティ影響と実験結果 (Results & Impact)
【実証・影響】As a result, we obtain a randomized classical algorithm for SVP that runs in $2^{0.5596...
...
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-09-02/iacr-2026-1844_meta.json) | [`PDF (arXiv)`](https://eprint.iacr.org/2026/1844.pdf) | [`TXT`](../../raw_data/2026-09-02/iacr-2026-1844_meta.json)

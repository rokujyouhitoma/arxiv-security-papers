---
type: "security-paper"
title: "SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing"
title_ja: "SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing（セキュリティ分析論文）"
description: "SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2602.01150v2"
tags:
  - "cs.AI"
  - "cs.CV"
  - "executive-summary"
  - "math.OC"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
timestamp: "2026-08-14T06:11:08Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-02-01/2602.01150_meta.json"
  published_date: "2026-02-01T10:51:53Z"
  authors:
    - "Jialong Sun"
    - "Zeming Wei"
    - "Jiaxuan Zou"
    - "Jiacheng Gong"
    - "Jie Fu"
    - "Chengyang Dong"
    - "Heng Xu"
    - "Jialong Li"
    - "Bo Liu"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing
### (日本語題名: SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2602.01150v2`](https://arxiv.org/abs/2602.01150v2) | Raw Meta = [`2602.01150_meta.json`](../../raw_data/2026-02-01/2602.01150_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing（セキュリティ分析論文）」（原題: SMI: Statistical Membership Inference for Reliable Unlearned Model Auditing / arXiv: 2602.01150v2）は、cs.LG 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, flaw)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, flaw)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2602.01150v2`
- **論文URL**: [https://arxiv.org/abs/2602.01150v2](https://arxiv.org/abs/2602.01150v2)
- **PDFリンク**: [https://arxiv.org/pdf/2602.01150v2.pdf](https://arxiv.org/pdf/2602.01150v2.pdf)
- **著者**: Jialong Sun, Zeming Wei, Jiaxuan Zou, Jiacheng Gong, Jie Fu, Chengyang Dong, Heng Xu, Jialong Li, Bo Liu
- **公開日時**: `2026-02-01T10:51:53Z`
- **カテゴリ**: `cs.LG, cs.AI, cs.CR, cs.CV, math.OC`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-02-01/2602.01150_meta.json) | [`PDF`](../../raw_data/2026-02-01/2602.01150.pdf) | [`TXT`](../../raw_data/2026-02-01/2602.01150.txt)

### Abstract (原文)
> Machine unlearning (MU) is essential for enforcing the right to be forgotten in machine learning systems. A key challenge of MU is how to reliably audit whether a model has truly forgotten specified training data. Membership Inference Attacks (MIAs) are widely used for unlearned model auditing, where samples that evade membership detection are regarded as successfully forgotten. We show this assumption is fundamentally flawed: failed membership inference does not imply true forgetting. We prove that unlearned samples occupy fundamentally different positions in the feature space than non-member samples, making this alignment bias unavoidable and unobservable, which leads to systematically optimistic evaluations of unlearning performance. Meanwhile, training shadow models for MIA incurs substantial computational overhead. To address both limitations, we propose Statistical Membership Inference (SMI), a training-free auditing framework that reformulates auditing as estimating the non-member mixture proportion in the unlearned feature distribution. Beyond estimating the forgetting rate, SMI also provides bootstrap reference ranges for quantified auditing reliability. Extensive experiments show that SMI consistently outperforms all MIA-based baselines, with no shadow model training required. Overall, SMI establishes a principled and efficient alternative to MIA-based auditing methods, with both theoretical guarantees and strong empirical performance.

---
type: "security-paper"
title: "Preventing Prompt Injection with Type-Directed Privilege Separation"
title_ja: "Preventing Prompt Injection with Type-Directed Privilege Separation（セキュリティ分析論文）"
description: "Preventing Prompt Injection with Type-Directed Privilege Separation（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2509.25926v2"
tags:
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
timestamp: "2026-08-14T06:11:10Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-09-30/2509.25926_meta.json"
  published_date: "2025-09-30T08:20:50Z"
  authors:
    - "Dennis Jacob"
    - "Emad Alghamdi"
    - "Zhanhao Hu"
    - "Basel Alomair"
    - "David Wagner"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Preventing Prompt Injection with Type-Directed Privilege Separation
### (日本語題名: Preventing Prompt Injection with Type-Directed Privilege Separation（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2509.25926v2`](https://arxiv.org/abs/2509.25926v2) | Raw Meta = [`2509.25926_meta.json`](../../raw_data/2025-09-30/2509.25926_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Preventing Prompt Injection with Type-Directed Privilege Separation（セキュリティ分析論文）」（原題: Preventing Prompt Injection with Type-Directed Privilege Separation / arXiv: 2509.25926v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2509.25926v2`
- **論文URL**: [https://arxiv.org/abs/2509.25926v2](https://arxiv.org/abs/2509.25926v2)
- **PDFリンク**: [https://arxiv.org/pdf/2509.25926v2.pdf](https://arxiv.org/pdf/2509.25926v2.pdf)
- **著者**: Dennis Jacob, Emad Alghamdi, Zhanhao Hu, Basel Alomair, David Wagner
- **公開日時**: `2025-09-30T08:20:50Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-09-30/2509.25926_meta.json) | [`PDF`](../../raw_data/2025-09-30/2509.25926.pdf) | [`TXT`](../../raw_data/2025-09-30/2509.25926.txt)

### Abstract (原文)
> Modern language models have enabled the development of agentic systems that achieve strong performance on reasoning-intensive tasks. Unfortunately, this has come with a security cost; these systems are vulnerable to prompt injection, a specialized attack where an adversary subverts the intended functionality of an agent by supplying an injected task of their own. Previous approaches address this challenge with detectors and fine-tuning defenses but are vulnerable to adaptive attacks. Other methods propose system-level defenses that guarantee security, but these are often based on techniques that prevent inter-component communication and thus are constrained in problem coverage. To this end, we introduce type-directed privilege separation, a new technique that expands the set of tasks that can be protected with system-level defenses. Our method works by converting untrusted data to a curated set of data types; unlike raw strings, each data type is limited in scope and content, eliminating the possibility for prompt injection. We evaluate our method across several case studies and find that designs using our principles can systematically prevent prompt injection attacks while featuring strong, non-trivial utility. Our approach is intuitive to understand and compatible with any language model.

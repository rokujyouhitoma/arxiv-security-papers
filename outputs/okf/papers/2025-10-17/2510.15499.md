---
type: "security-paper"
title: "HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment"
title_ja: "HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment（セキュリティ分析論文）"
description: "HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2510.15499v2"
tags:
  - "cybersecurity"
  - "executive-summary"
  - "cs.CR"
  - "arXiv"
timestamp: "2026-08-14T06:11:10Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-10-17/2510.15499_meta.json"
  published_date: "2025-10-17T10:13:44Z"
  authors:
    - "Yuexiao Liu"
    - "Lijun Li"
    - "Xingjun Wang"
    - "Jing Shao"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment
### (日本語題名: HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2510.15499v2`](https://arxiv.org/abs/2510.15499v2) | Raw Meta = [`2510.15499_meta.json`](../../raw_data/2025-10-17/2510.15499_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment（セキュリティ分析論文）」（原題: HarmRLVR: Weaponizing Verifiable Rewards for Harmful LLM Alignment / arXiv: 2510.15499v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat, risk)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, threat, risk)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2510.15499v2`
- **論文URL**: [https://arxiv.org/abs/2510.15499v2](https://arxiv.org/abs/2510.15499v2)
- **PDFリンク**: [https://arxiv.org/pdf/2510.15499v2.pdf](https://arxiv.org/pdf/2510.15499v2.pdf)
- **著者**: Yuexiao Liu, Lijun Li, Xingjun Wang, Jing Shao
- **公開日時**: `2025-10-17T10:13:44Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-10-17/2510.15499_meta.json) | [`PDF`](../../raw_data/2025-10-17/2510.15499.pdf) | [`TXT`](../../raw_data/2025-10-17/2510.15499.txt)

### Abstract (原文)
> Recent advancements in Reinforcement Learning with Verifiable Rewards (RLVR) have gained significant attention due to their objective and verifiable reward signals, demonstrating strong performance in reasoning and code generation tasks. However, the potential safety risks associated with RLVR remain underexplored. This paper presents HarmRLVR, the first systematic investigation into the alignment reversibility risk of RLVR. We show that safety alignment can be rapidly reversed using GRPO with merely 64 harmful prompts without responses, causing models to readily comply with harmful instructions. Across five models from Llama, Qwen, and DeepSeek, we empirically demonstrate that RLVR-based attacks elevate the average harmfulness score to 4.94 with an attack success rate of 96.01\%, significantly outperforming harmful fine-tuning while preserving general capabilities. Our findings reveal that RLVR can be efficiently exploited for harmful alignment, posing serious threats to open-source model safety. Please see our code at https://github.com/lyxx2535/HarmRLVR.

---
type: "security-paper"
title: "Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage"
title_ja: "Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage（セキュリティ分析論文）"
description: "Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2606.19076v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
timestamp: "2026-08-14T04:52:55Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-06-17/2606.19076_meta.json"
  published_date: "2026-06-17T13:47:34Z"
  authors:
    - "Faruk Alpay"
    - "Taylan Alpay"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage
### (日本語題名: Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2606.19076v1`](https://arxiv.org/abs/2606.19076v1) | Raw Meta = [`2606.19076_meta.json`](../../raw_data/2026-06-17/2606.19076_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage（セキュリティ分析論文）」（原題: Compute-Budgeted Exploitability Evidence Graphs for Prospective Vulnerability Triage / arXiv: 2606.19076v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, exploit, leak)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, exploit, leak)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2606.19076v1`
- **論文URL**: [https://arxiv.org/abs/2606.19076v1](https://arxiv.org/abs/2606.19076v1)
- **PDFリンク**: [https://arxiv.org/pdf/2606.19076v1.pdf](https://arxiv.org/pdf/2606.19076v1.pdf)
- **著者**: Faruk Alpay, Taylan Alpay
- **公開日時**: `2026-06-17T13:47:34Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-06-17/2606.19076_meta.json) | [`PDF`](../../raw_data/2026-06-17/2606.19076.pdf) | [`TXT`](../../raw_data/2026-06-17/2606.19076.txt)

### Abstract (原文)
> Defenders cannot patch every newly disclosed vulnerability at once, so exploitability prediction must be evaluated prospectively rather than retrospectively. We study compute-budgeted vulnerability triage in which each CVE is scored only from public evidence visible by a fixed decision time. Advisories, exploit archives, fix commits, and hacker-community discourse are represented as a temporal evidence graph; a budgeted selector admits only a few evidence documents per CVE, and every score is paired with an auditable certificate listing the supporting signals, timestamps, source layers, and leakage flags. On 12012 prospective CVEs from public sources, budgeted evidence selection raises leakage-safe prospective recall@50 from 0.010 for a severity-only baseline to 0.026, while two evidence documents per CVE capture most of the value. A strong cross-encoder reranker lowers prospective recall to 0.016, showing that semantic relevance to a CVE is not the same as evidence of exploitation. Most importantly, a naive random split with unfiltered evidence inflates apparent prospective recall by 8.5x and EPSS-high recall by 5.0x. The main contribution is a leakage-safe evaluation protocol and reproducible evidence certificates for contestable vulnerability-prioritization claims.

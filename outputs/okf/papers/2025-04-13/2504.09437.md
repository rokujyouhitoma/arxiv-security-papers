---
type: "security-paper"
title: "PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices"
title_ja: "PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices（セキュリティ分析論文）"
description: "PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2504.09437v1"
tags:
  - "cs.CR"
  - "arXiv"
  - "cybersecurity"
  - "eess.SP"
  - "executive-summary"
timestamp: "2026-08-14T07:15:32Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-04-13/2504.09437_meta.json"
  published_date: "2025-04-13T05:14:17Z"
  authors:
    - "Hamid Amiriara"
    - "Mahtab Mirmohseni"
    - "Rahim Tafazolli"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices
### (日本語題名: PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2504.09437v1`](https://arxiv.org/abs/2504.09437v1) | Raw Meta = [`2504.09437_meta.json`](../../raw_data/2025-04-13/2504.09437_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices（セキュリティ分析論文）」（原題: PLS-Assisted Offloading for Edge Computing-Enabled Post-Quantum Security in Resource-Constrained Devices / arXiv: 2504.09437v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, crypto)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, crypto)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2504.09437v1`
- **論文URL**: [https://arxiv.org/abs/2504.09437v1](https://arxiv.org/abs/2504.09437v1)
- **PDFリンク**: [https://arxiv.org/pdf/2504.09437v1.pdf](https://arxiv.org/pdf/2504.09437v1.pdf)
- **著者**: Hamid Amiriara, Mahtab Mirmohseni, Rahim Tafazolli
- **公開日時**: `2025-04-13T05:14:17Z`
- **カテゴリ**: `cs.CR, eess.SP`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-04-13/2504.09437_meta.json) | [`PDF`](../../raw_data/2025-04-13/2504.09437.pdf) | [`TXT`](../../raw_data/2025-04-13/2504.09437.txt)

### Abstract (原文)
> With the advent of post-quantum cryptography (PQC) standards, it has become imperative for resource-constrained devices (RCDs) in the Internet of Things (IoT) to adopt these quantum-resistant protocols. However, the high computational overhead and the large key sizes associated with PQC make direct deployment on such devices impractical. To address this challenge, we propose an edge computing-enabled PQC framework that leverages a physical-layer security (PLS)-assisted offloading strategy, allowing devices to either offload intensive cryptographic tasks to a post-quantum edge server (PQES) or perform them locally. Furthermore, to ensure data confidentiality within the edge domain, our framework integrates two PLS techniques: offloading RCDs employ wiretap coding to secure data transmission, while non-offloading RCDs serve as friendly jammers by broadcasting artificial noise to disrupt potential eavesdroppers. Accordingly, we co-design the computation offloading and PLS strategy by jointly optimizing the device transmit power, PQES computation resource allocation, and offloading decisions to minimize overall latency under resource constraints. Numerical results demonstrate significant latency reductions compared to baseline schemes, confirming the scalability and efficiency of our approach for secure PQC operations in IoT networks.

---
type: "security-paper"
title: "Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry"
title_ja: "Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry（セキュリティ分析論文）"
description: "Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2607.07075v2"
tags:
  - "Cryptography"
  - "arXiv"
  - "cs.CR"
  - "cybersecurity"
  - "executive-summary"
  - "network-protocol"
timestamp: "2026-08-20T20:42:01Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-07-08/2607.07075_meta.json"
  published_date: "2026-07-08T07:07:08Z"
  authors:
    - "Abdul Rahman"
    - "Eranga Bandara"
    - "Sachin Shetty"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry
### (日本語題名: Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2607.07075v2`](https://arxiv.org/abs/2607.07075v2) | Raw Meta = [`2607.07075_meta.json`](../../raw_data/2026-07-08/2607.07075_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry（セキュリティ分析論文）」（原題: Cyber Dynamics I: Finite Macrostates for Behavioral Anomaly Detection in Network Telemetry / arXiv: 2607.07075v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, drift)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, drift)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2607.07075v2`
- **論文URL**: [https://arxiv.org/abs/2607.07075v2](https://arxiv.org/abs/2607.07075v2)
- **PDFリンク**: [https://arxiv.org/pdf/2607.07075v2.pdf](https://arxiv.org/pdf/2607.07075v2.pdf)
- **著者**: Abdul Rahman, Eranga Bandara, Sachin Shetty
- **公開日時**: `2026-07-08T07:07:08Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-07-08/2607.07075_meta.json) | [`PDF`](../../raw_data/2026-07-08/2607.07075.pdf) | [`TXT`](../../raw_data/2026-07-08/2607.07075.txt)

### Abstract (原文)
> Entropy-based methods have long been used for network anomaly detection, but most existing approaches treat entropy as a scalar statistic on narrow observables rather than as part of a broader behavioral state-space for cyber systems. We propose a finite-dimensional macrostate framework for network telemetry, instantiated over the Canonical Security Telemetry Substrate (CSTS), so that coarse-graining is performed over persistent entities, typed relations, and temporal state rather than isolated event records. The resulting macrostate captures activity, distributional disorder, structural organization, temporal volatility, persistence, and deviation from benign baselines. Rather than scoring only unusual states, we model window-to-window macrostate transitions and define regime structure, stability, and anomalous change. This supports discrimination between benign workload drift and adversarial reorganization. We evaluate the framework on benchmark network telemetry datasets and compare it against Shannon-, Rényi-, and Tsallis-style entropy baselines, as well as standard anomaly detectors. The proposed representation improves anomaly discrimination and supports more interpretable behavioral analysis of cyber telemetry.

---
type: "security-paper"
title: "AICrypto: Evaluating Cryptography Capabilities of Large Language Models"
title_ja: "AICrypto: Evaluating Cryptography Capabilities of 大規模言語モデル"
description: "AICrypto: Evaluating Cryptography Capabilities of 大規模言語モデル — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2507.09580v6"
tags:
  - "cs.CR"
  - "arXiv"
  - "cybersecurity"
  - "executive-summary"
timestamp: "2026-08-14T07:15:31Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-07-13/2507.09580_meta.json"
  published_date: "2025-07-13T11:11:01Z"
  authors:
    - "Yu Wang"
    - "Yijian Liu"
    - "Liheng Ji"
    - "Han Luo"
    - "Wenjie Li"
    - "Xiaofei Zhou"
    - "Chiyun Feng"
    - "Puji Wang"
    - "Yuhan Cao"
    - "Geyuan Zhang"
    - "Xiaojian Li"
    - "Rongwu Xu"
    - "Yilei Chen"
    - "Tianxing He"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# AICrypto: Evaluating Cryptography Capabilities of Large Language Models
### (日本語題名: AICrypto: Evaluating Cryptography Capabilities of 大規模言語モデル)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2507.09580v6`](https://arxiv.org/abs/2507.09580v6) | Raw Meta = [`2507.09580_meta.json`](../../raw_data/2025-07-13/2507.09580_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「AICrypto: Evaluating Cryptography Capabilities of 大規模言語モデル」（原題: AICrypto: Evaluating Cryptography Capabilities of Large Language Models / arXiv: 2507.09580v6）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, exploit, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, exploit, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2507.09580v6`
- **論文URL**: [https://arxiv.org/abs/2507.09580v6](https://arxiv.org/abs/2507.09580v6)
- **PDFリンク**: [https://arxiv.org/pdf/2507.09580v6.pdf](https://arxiv.org/pdf/2507.09580v6.pdf)
- **著者**: Yu Wang, Yijian Liu, Liheng Ji, Han Luo, Wenjie Li, Xiaofei Zhou, Chiyun Feng, Puji Wang, Yuhan Cao, Geyuan Zhang, Xiaojian Li, Rongwu Xu, Yilei Chen, Tianxing He
- **公開日時**: `2025-07-13T11:11:01Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-07-13/2507.09580_meta.json) | [`PDF`](../../raw_data/2025-07-13/2507.09580.pdf) | [`TXT`](../../raw_data/2025-07-13/2507.09580.txt)

### Abstract (原文)
> We build \textbf{AICrypto}, a comprehensive benchmark designed to evaluate the cryptography capabilities of large language models (LLMs). The benchmark comprises 135 multiple-choice questions, 150 capture-the-flag challenges, and 30 proof problems, covering a broad range of skills from knowledge memorization to vulnerability exploitation and formal reasoning. All tasks are carefully reviewed or constructed by cryptography experts to improve correctness and rigor. For each proof problem, we provide detailed scoring rubrics and reference solutions that enable automated grading, achieving high correlation with human expert evaluations. We introduce strong human expert performance baselines for comparison across all task types. Our evaluation of 17 leading LLMs reveals that state-of-the-art models match or even surpass human experts in memorizing cryptographic concepts, exploiting common vulnerabilities, and routine proofs. However, our analysis reveals that they still lack a deep understanding of abstract mathematical concepts and struggle with tasks that require multi-step reasoning and dynamic analysis. We hope this work could provide insights for future research on LLMs in cryptographic applications. Our code and dataset are available at https://github.com/wangyu-ovo/aicrypto-agent.

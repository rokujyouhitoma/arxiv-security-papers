---
type: "security-paper"
title: "SkillGuard: A Permission-Centric Framework for Agent Skill Security"
title_ja: "SkillGuard: A Permission-Centric Framework for Agent Skill Security（セキュリティ分析論文）"
description: "SkillGuard: A Permission-Centric Framework for Agent Skill Security（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2606.03024v2"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.SE"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:53Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-06-02/2606.03024_meta.json"
  published_date: "2026-06-02T02:01:53Z"
  authors:
    - "Shidong Pan"
    - "Xiaoyu Sun"
    - "Tianyi Zhang"
    - "Dianshu Liao"
    - "Kaiwen Yang"
    - "Zhenchang Xing"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# SkillGuard: A Permission-Centric Framework for Agent Skill Security
### (日本語題名: SkillGuard: A Permission-Centric Framework for Agent Skill Security（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2606.03024v2`](https://arxiv.org/abs/2606.03024v2) | Raw Meta = [`2606.03024_meta.json`](../../raw_data/2026-06-02/2606.03024_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「SkillGuard: A Permission-Centric Framework for Agent Skill Security（セキュリティ分析論文）」（原題: SkillGuard: A Permission-Centric Framework for Agent Skill Security / arXiv: 2606.03024v2）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2606.03024v2`
- **論文URL**: [https://arxiv.org/abs/2606.03024v2](https://arxiv.org/abs/2606.03024v2)
- **PDFリンク**: [https://arxiv.org/pdf/2606.03024v2.pdf](https://arxiv.org/pdf/2606.03024v2.pdf)
- **著者**: Shidong Pan, Xiaoyu Sun, Tianyi Zhang, Dianshu Liao, Kaiwen Yang, Zhenchang Xing
- **公開日時**: `2026-06-02T02:01:53Z`
- **カテゴリ**: `cs.CR, cs.SE`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-06-02/2606.03024_meta.json) | [`PDF`](../../raw_data/2026-06-02/2606.03024.pdf) | [`TXT`](../../raw_data/2026-06-02/2606.03024.txt)

### Abstract (原文)
> Skills extend LLM agents with reusable instructions, scripts, data, and tool bindings. This shift makes skills a new security principal in agent systems: a skill can alter the agent's reasoning before any tool is called, and it can also steer the agent toward actions with concrete side effects. However, current skill ecosystems lack a permission model that captures this dual role. Existing defenses either inspect skill files before use or constrain individual tool calls during execution, leaving the connection between skill-level intent, contextual influence, and runtime behavior weakly governed. In this paper, we present SkillGuard, a skill-centric permission framework that treats skills as permission-bearing executable artifacts. SkillGuard introduces a dual-plane governance model that jointly regulates context influence and action side effects through skill manifests, runtime permission control, user interaction, and policy enforcement. We evaluate the permission taxonomy expressiveness on 1,260 real-world skills, and 99.93% of observed protected objects are covered. In adversarial evaluations on SkillInject dataset, SkillGuard reduces attack success rate from 35.3% to 20.7% for contextual injections and from 36.7% to 18.0% for obvious injections, while decently maintaining benign task completion. These results suggest that SkillGuard, as a skill-centric permission framework, can provide a practical foundation for improving the security of agent skill ecosystems.

---
type: "security-paper"
title: "Tatemae: Detecting Alignment Faking via Tool Selection in LLMs"
title_ja: "Tatemae: Detecting Alignment Faking via Tool Selection in LLMs（セキュリティ分析論文）"
description: "Tatemae: Detecting Alignment Faking via Tool Selection in LLMs（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2604.26511v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.AI"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:55Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-04-29/2604.26511_meta.json"
  published_date: "2026-04-29T10:20:08Z"
  authors:
    - "Matteo Leonesi"
    - "Francesco Belardinelli"
    - "Flavio Corradini"
    - "Marco Piangerelli"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Tatemae: Detecting Alignment Faking via Tool Selection in LLMs
### (日本語題名: Tatemae: Detecting Alignment Faking via Tool Selection in LLMs（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2604.26511v1`](https://arxiv.org/abs/2604.26511v1) | Raw Meta = [`2604.26511_meta.json`](../../raw_data/2026-04-29/2604.26511_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Tatemae: Detecting Alignment Faking via Tool Selection in LLMs（セキュリティ分析論文）」（原題: Tatemae: Detecting Alignment Faking via Tool Selection in LLMs / arXiv: 2604.26511v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, privacy, security)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: vulnerability, privacy, security)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2604.26511v1`
- **論文URL**: [https://arxiv.org/abs/2604.26511v1](https://arxiv.org/abs/2604.26511v1)
- **PDFリンク**: [https://arxiv.org/pdf/2604.26511v1.pdf](https://arxiv.org/pdf/2604.26511v1.pdf)
- **著者**: Matteo Leonesi, Francesco Belardinelli, Flavio Corradini, Marco Piangerelli
- **公開日時**: `2026-04-29T10:20:08Z`
- **カテゴリ**: `cs.CR, cs.AI`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-04-29/2604.26511_meta.json) | [`PDF`](../../raw_data/2026-04-29/2604.26511.pdf) | [`TXT`](../../raw_data/2026-04-29/2604.26511.txt)

### Abstract (原文)
> Alignment faking (AF) occurs when an LLM strategically complies with training objectives to avoid value modification, reverting to prior preferences once monitoring is lifted. Current detection methods focus on conversational settings and rely primarily on Chain-of-Thought (CoT) analysis, which provides a reliable signal when strategic reasoning surfaces, but cannot distinguish deception from capability failures if traces are absent or unfaithful. We formalize AF as a composite behavioural event and detect it through observable tool selection, where the LLM selects the safe tool when unmonitored, but switches to the unsafe tool under monitoring that rewards helpfulness over safety, while its reasoning still acknowledges the safe choice. We release a dataset of 108 enterprise IT scenarios spanning Security, Privacy, and Integrity domains under Corruption and Sabotage pressures. Evaluating six frontier LLMs across five independent runs, we find mean AF detection rates between 3.5% and 23.7%, with vulnerability profiles varying by domain and pressure type. These results suggest that susceptibility reflects training methodology rather than capability alone.

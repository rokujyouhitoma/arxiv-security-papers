---
type: "security-paper"
title: "PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents"
title_ja: "PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents（セキュリティ分析論文）"
description: "PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2605.21694v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.AI"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:54Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-05-20/2605.21694_meta.json"
  published_date: "2026-05-20T19:52:24Z"
  authors:
    - "Sidnei Barbieri"
    - "Ágney Lopes Roth Ferraz"
    - "Lourenço Alves Pereira Júnior"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents
### (日本語題名: PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2605.21694v1`](https://arxiv.org/abs/2605.21694v1) | Raw Meta = [`2605.21694_meta.json`](../../raw_data/2026-05-20/2605.21694_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents（セキュリティ分析論文）」（原題: PocketAgents: A Manifest-Driven Library of Autonomous Defense Agents / arXiv: 2605.21694v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2605.21694v1`
- **論文URL**: [https://arxiv.org/abs/2605.21694v1](https://arxiv.org/abs/2605.21694v1)
- **PDFリンク**: [https://arxiv.org/pdf/2605.21694v1.pdf](https://arxiv.org/pdf/2605.21694v1.pdf)
- **著者**: Sidnei Barbieri, Ágney Lopes Roth Ferraz, Lourenço Alves Pereira Júnior
- **公開日時**: `2026-05-20T19:52:24Z`
- **カテゴリ**: `cs.CR, cs.AI`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-05-20/2605.21694_meta.json) | [`PDF`](../../raw_data/2026-05-20/2605.21694.pdf) | [`TXT`](../../raw_data/2026-05-20/2605.21694.txt)

### Abstract (原文)
> Connecting large language models (LLMs) to defensive enforcement requires more than asking a model whether an attack is happening. A defender must decide which model outputs may change the system state, which outputs must be rejected, and how failures should be recorded. We present PocketAgents, a manifest-driven library of autonomous defense agents. Each agent is installed as three data files: a manifest, a prompt, and a runtime context. The shared runtime gives the agent bounded telemetry access and accepts only typed reports whose requested action appears in the manifest. We implemented PocketAgents on top of a cyber arena (Perry), a cyber-deception testbed, and evaluated two agents, Command and Control and Exfiltration, in 18 closed-loop trials of a DarkSide-inspired attack on a small enterprise topology. Thirteen trials produced validated network-block actions and contained the attack; four failed schema validation; one produced a valid no-action decision. The experiments show that a typed boundary makes LLM-driven defense measurable, extensible, and attributable.

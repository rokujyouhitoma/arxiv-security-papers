---
type: "security-paper"
title: "PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features"
title_ja: "PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features（セキュリティ分析論文）"
description: "PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2510.14005v3"
tags:
  - "executive-summary"
  - "cybersecurity"
  - "cs.CR"
  - "arXiv"
  - "cs.LG"
timestamp: "2026-08-14T06:11:10Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-10-15/2510.14005_meta.json"
  published_date: "2025-10-15T18:34:49Z"
  authors:
    - "Wei Zou"
    - "Yupei Liu"
    - "Yanting Wang"
    - "Ying Chen"
    - "Neil Gong"
    - "Jinyuan Jia"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features
### (日本語題名: PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2510.14005v3`](https://arxiv.org/abs/2510.14005v3) | Raw Meta = [`2510.14005_meta.json`](../../raw_data/2025-10-15/2510.14005_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features（セキュリティ分析論文）」（原題: PIShield: Detecting Prompt Injection Attacks via Intrinsic LLM Features / arXiv: 2510.14005v3）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2510.14005v3`
- **論文URL**: [https://arxiv.org/abs/2510.14005v3](https://arxiv.org/abs/2510.14005v3)
- **PDFリンク**: [https://arxiv.org/pdf/2510.14005v3.pdf](https://arxiv.org/pdf/2510.14005v3.pdf)
- **著者**: Wei Zou, Yupei Liu, Yanting Wang, Ying Chen, Neil Gong, Jinyuan Jia
- **公開日時**: `2025-10-15T18:34:49Z`
- **カテゴリ**: `cs.CR, cs.LG`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-10-15/2510.14005_meta.json) | [`PDF`](../../raw_data/2025-10-15/2510.14005.pdf) | [`TXT`](../../raw_data/2025-10-15/2510.14005.txt)

### Abstract (原文)
> LLM-integrated applications are vulnerable to prompt injection attacks, where an attacker contaminates the input to inject malicious instructions, causing the LLM to follow the attacker's intent instead of the original user's. Existing prompt injection detection methods often have sub-optimal performance and/or high computational overhead. In this work, we propose PIShield, an effective and efficient detection method based on the observation that instruction-tuned LLMs internally encode distinguishable signals for prompts containing injected instructions. PIShield leverages residual-stream representations and a simple linear classifier to detect prompt injection, without expensive model fine-tuning or response generation. We conduct extensive evaluations on a diverse set of short- and long-context benchmarks. The results show that PIShield consistently achieves low false positive and false negative rates, significantly outperforming existing baselines. These findings demonstrate that internal representations of instruction-tuned LLMs provide a powerful and practical foundation for prompt injection detection in real-world applications.

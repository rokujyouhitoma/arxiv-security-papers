---
type: "security-paper"
title: "Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing"
title_ja: "Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing（セキュリティ分析論文）"
description: "Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2605.11202v1"
tags:
  - "arXiv"
  - "executive-summary"
  - "cs.SE"
  - "cs.AI"
  - "cs.LG"
  - "cs.CR"
  - "cybersecurity"
timestamp: "2026-08-14T04:52:54Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2026-05-11/2605.11202_meta.json"
  published_date: "2026-05-11T20:10:08Z"
  authors:
    - "Yunze Zhao"
    - "Yibo Zhao"
    - "Yuchen Zhang"
    - "Zaoxing Liu"
    - "Michelle L. Mazurek"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing
### (日本語題名: Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2605.11202v1`](https://arxiv.org/abs/2605.11202v1) | Raw Meta = [`2605.11202_meta.json`](../../raw_data/2026-05-11/2605.11202_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing（セキュリティ分析論文）」（原題: Continuous Discovery of Vulnerabilities in LLM Serving Systems with Fuzzing / arXiv: 2605.11202v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: security, llm)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2605.11202v1`
- **論文URL**: [https://arxiv.org/abs/2605.11202v1](https://arxiv.org/abs/2605.11202v1)
- **PDFリンク**: [https://arxiv.org/pdf/2605.11202v1.pdf](https://arxiv.org/pdf/2605.11202v1.pdf)
- **著者**: Yunze Zhao, Yibo Zhao, Yuchen Zhang, Zaoxing Liu, Michelle L. Mazurek
- **公開日時**: `2026-05-11T20:10:08Z`
- **カテゴリ**: `cs.CR, cs.AI, cs.LG, cs.SE`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2026-05-11/2605.11202_meta.json) | [`PDF`](../../raw_data/2026-05-11/2605.11202.pdf) | [`TXT`](../../raw_data/2026-05-11/2605.11202.txt)

### Abstract (原文)
> LLM inference and serving systems have become security-critical infrastructure; however, many of their most concerning failures arise from the serving layer rather than from model behavior alone. Modern inference engines combine KV cache, batching, prefix sharing, speculative decoding, adapters, and multi-tenant scheduling, creating shared-state behavior that only emerges under realistic concurrent workloads and is missed by standard model, safety, and API tests. We present GRIEF, a greybox fuzzer for LLM inference engines that treats timed multi-request traces as first-class inputs, uses lightweight oracles to detect crashes, hangs, performance pathologies, and silent output corruption, and applies controlled replay with log-probability checks to confirm reproducible serving-layer failures. Across early campaigns on vLLM and SGLang, GRIEF discovers 15 vulnerabilities, 10 confirmed by engine developers, including 2 CVEs, spanning KV-cache isolation failures, cross-request performance interference, and crash or liveness bugs. These results show that concurrency, caching, and state reuse can induce silent cross-request contamination, noisy-neighbor denial of service, and delayed crashes without malformed inputs or explicit server errors, making concurrent serving behavior a first-class security and reliability boundary for LLM infrastructure.

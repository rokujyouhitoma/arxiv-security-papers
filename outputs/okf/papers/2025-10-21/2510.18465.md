---
type: "security-paper"
title: "PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks"
title_ja: "PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks（セキュリティ分析論文）"
description: "PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks（セキュリティ分析論文） — 課題分析と防御モデルの検証"
resource: "https://arxiv.org/abs/2510.18465v1"
tags:
  - "cybersecurity"
  - "executive-summary"
  - "cs.CR"
  - "arXiv"
timestamp: "2026-08-14T06:11:10Z"
provenance:
  source: "arxiv.org"
  raw_meta_file: "../../raw_data/2025-10-21/2510.18465_meta.json"
  published_date: "2025-10-21T09:42:46Z"
  authors:
    - "Spencer King"
    - "Irfan Ozen"
    - "Karthika Subramani"
    - "Saranyan Senthivel"
    - "Phani Vadrevu"
    - "Roberto Perdisci"
trust:
  attestation: "processed_by: arxiv-security-agent"
  confidence: "high"
---

# PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks
### (日本語題名: PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks（セキュリティ分析論文）)

> [!NOTE]
> **OKF Metadata**: Type = `security-paper` | arXiv ID = [`2510.18465v1`](https://arxiv.org/abs/2510.18465v1) | Raw Meta = [`2510.18465_meta.json`](../../raw_data/2025-10-21/2510.18465_meta.json)

## エグゼクティブサマリー (Executive Summary)

### 1. 概要 (Overview & Key Finding)
本論文「PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks（セキュリティ分析論文）」（原題: PP3D: An In-Browser Vision-Based Defense Against Web Behavior Manipulation Attacks / arXiv: 2510.18465v1）は、cs.CR 分野における最新セキュリティ研究成果を取り扱っています。

**概要**: 本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, exploit, privacy)

### 2. 背景とセキュリティ上の課題 (Background & Problem)
本研究はサイバー脅威環境におけるセキュリティ構造・脆弱性の検証を目的としています。(主要検出項目: attack, exploit, privacy)

### 3. 提案アプローチ・技術革新 (Technical Innovation)
理論的解析および実証実験データセットに基づく検出・評価メカニズムを新規構築しています。

### 4. セキュリティ影響と実験結果 (Results & Impact)
実験結果より、脆弱性検出精度の向上、誤検知率の低減、あるいは理論的安全性の証明が確認されました。

### 5. 経営層・セキュリティ管理者向け推奨アクション (Executive Recommendations)
- 組織のセキュリティ設計およびリスク評価基準への影響確認
- 技術チームによる検証実験および対策パッチ/設定の展開検討
- 関連する暗号プロトコル・認証ロジックの脆弱性監査の実施

---

## 原論文情報 (Original Paper Metadata & Raw Data)

- **arXiv ID**: `2510.18465v1`
- **論文URL**: [https://arxiv.org/abs/2510.18465v1](https://arxiv.org/abs/2510.18465v1)
- **PDFリンク**: [https://arxiv.org/pdf/2510.18465v1.pdf](https://arxiv.org/pdf/2510.18465v1.pdf)
- **著者**: Spencer King, Irfan Ozen, Karthika Subramani, Saranyan Senthivel, Phani Vadrevu, Roberto Perdisci
- **公開日時**: `2025-10-21T09:42:46Z`
- **カテゴリ**: `cs.CR`
- **保存済みRawデータ**: [`JSON`](../../raw_data/2025-10-21/2510.18465_meta.json) | [`PDF`](../../raw_data/2025-10-21/2510.18465.pdf) | [`TXT`](../../raw_data/2025-10-21/2510.18465.txt)

### Abstract (原文)
> Web-based behavior-manipulation attacks (BMAs) - such as scareware, fake software downloads, tech support scams, etc. - are a class of social engineering (SE) attacks that exploit human decision-making vulnerabilities. These attacks remain under-studied compared to other attacks such as information harvesting attacks (e.g., phishing) or malware infections. Prior technical work has primarily focused on measuring BMAs, offering little in the way of generic defenses. To address this gap, we introduce Pixel Patrol 3D (PP3D), the first end-to-end browser framework for discovering, detecting, and defending against behavior-manipulating SE attacks in real time. PP3D consists of a visual detection model implemented within a browser extension, which deploys the model client-side to protect users across desktop and mobile devices while preserving privacy. Our evaluation shows that PP3D can achieve above 99% detection rate at 1% false positives, while maintaining good latency and overhead performance across devices. Even when faced with new BMA samples collected months after training the detection model, our defense system can still achieve above 97% detection rate at 1% false positives. These results demonstrate that our framework offers a practical, effective, and generalizable defense against a broad and evolving class of web behavior-manipulation attacks.

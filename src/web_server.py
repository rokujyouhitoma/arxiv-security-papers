...
from mcp_server import dispatch_tool, handle_get_latest_trends, handle_get_paper_summary
...
class ArxivWebServerHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/search":
            ...
            results, profile = VECTOR_ENGINE.search_with_profile(
        ...
        if path.startswith("/api/paper/"):
            ...
            self._send_json_response(
                res, status_code=200 if res.get("status") == "success" else 404
            )
            return
        ...
        if path in ["/", "/search"] or not os.path.exists(

...
def generate_japanese_executive_summary(paper):
    ...
    overview_desc = (
        ...
        f"{paper['primary_category']} 分野における最新セキュリティ研究成果を取り扱っています。"
    )
...
def build_okf_from_raw(raw_meta_path, workspace_dir, config):
    ...
    rel_raw_meta_from_okf = os.path.relpath(
        raw_meta_path, os.path.dirname(okf_file_path)
    )
...
def generate_per_run_summary(processed_items, workspace_dir, config):
    ...
    for idx, item in enumerate(processed_items, 1):
        ...
        row_str = (
            f"| {idx} | `{item['date_str']}` | [{clean_title_en}]({rel_okf}) | "
            f"{clean_title_ja} | [`{p['arxiv_id']}`]({p['abs_url']}) | {clean_one_liner} |"
        )
        rows.append(row_str)
        ...
        for day in sorted(os.listdir(okf_root)):
            ...
            if os.path.isdir(day_dir):
                ...
                for idx, pf in enumerate(paper_files, 1):
                    ...
                    title = (
                        title_match.group(1) if title_match else os.path.basename(pf)
                    )
                    title_ja = (
                        title_ja_match.group(1)
                        if title_ja_match
                        else translate_title_ja(title)
                    )
                    ...
                    row_str = (
                        f"| {idx} | [{c_title}]({rel_okf}) | {c_title_ja} | "
                        f"[`{arxiv_id}`](https://arxiv.org/abs/{arxiv_id}) | {c_desc} |"
                    )
                    rows.append(row_str)
...
def build_summary_table_md(paper_files, filepath):
    ...
    for idx, pf in enumerate(paper_files, 1):
        ...
        row_str = (
            f"| {idx} | `{pub_date}` | [{c_title}]({rel_okf}) | "
            f"{c_title_ja} | [`{arxiv_id}`](https://arxiv.org/abs/{arxiv_id}) | {c_desc} |"
        )
        rows.append(row_str)
...
def main():
    parser = argparse.ArgumentParser(
        description="arXiv Security Papers OKF & Summary Generator"
    )
    ...
    max_results = (
        ...
        else config["arxiv"].get("max_results_per_run", 3500)
    )
    print(
        f"[{datetime.now().isoformat()}] Fetching papers from arXiv (query={query}, max_results={max_results})..."
    )
    ...
    print(
        f"[{datetime.now().isoformat()}] Downloading PDFs and converting to TXT "
        f"for {len(pdf_fetch_tasks)} papers with 15 parallel threads..."
    )

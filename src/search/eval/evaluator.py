...
class SearchEvaluator:
    ...
    def generate_markdown_report(self, eval_result: Dict[str, Any]) -> str:
        ...
        lines = [
            f"# 検索エンジン評価レポート (IR Quality Benchmark Report)",
            f"",
            f"## 1. 総合評価サマリー (Overall Metrics at K={s['top_k']})",
            f"",
            f"| 評価指標 (Metric) | スコア (Score) | 理想値 (Ideal) | 説明 (Description) |",
            f"| :--- | :---: | :---: | :--- |",
            ...
            f"| **Mean Precision@{s['top_k']} (適合率)** | **{s['mean_precision_at_k']}** | 1.0000 | 上位 {s['top_k']} 件中、正解文書が含まれる割合 |",
            f"| **Mean Recall@{s['top_k']} (再現率)** | **{s['mean_recall_at_k']}** | 1.0000 | 全正解文書中、上位 {s['top_k']} 件に取得できた割合 |",
            f"| **Mean F1-Score (F値)** | **{s['mean_f1_score']}** | 1.0000 | 適合率と再現率の調和平均 |",
            f"",
            f"---",
            f"",
            f"## 2. クエリ別詳細結果 (Query Breakdown)",
            f"",
        ...
        for q in details:
            lines.append(
                f"| {q['query_id']} | `{q['category']}` | {q['query_text']} | {q['precision_at_k']} | {q['recall_at_k']} | {q['f1_score']} | {q['average_precision']} | {q['reciprocal_rank']} | {q['ndcg_at_k']} |"
            )

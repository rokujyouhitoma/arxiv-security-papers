...
# ---------------------------------------------------------------------------
# Knowledge Base for Automated Defense Synthesis
# ---------------------------------------------------------------------------
...
CWE_DEFENSE_MAP: Dict[str, Dict[str, Any]] = {
    ...
    "CWE-79": {
        ...
        "secure_alternative": "textContent = sanitized_input or DOMPurify.sanitize()",
...
TOOLS_MANIFEST = [
    {
        ...
        "description": "Synthesize a ready-to-use Semgrep YAML rule from a CWE ID or vulnerability pattern for CI/CD DevSecOps pipelines.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "cwe_id": {
    ...
    },
    {
        ...
        "description": "Generate an academic-aligned secure code remediation patch and diff for a vulnerable code snippet.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "code": {
    ...
    },
    {
        ...
        "description": "Evaluate the repository defense coverage across MITRE ATT&CK tactics and NIST SP 800-53 controls.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "declared_defenses": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "List of active defenses (e.g. ['pickle-free', 'ast-guard', 'parameterized-queries', 'commonpath-traversal-guard', 'zero-dependency'])",
                },
            },
...
def handle_generate_semgrep_rule(params: Dict[str, Any]) -> Dict[str, Any]:
  ...
  - id: {rule_id}
    patterns:
      - pattern: {pattern}
    message: "Detected potential {cwe_id} ({data.get('name', 'Vulnerability')}). Remediation: {data.get('secure_alternative', 'Use safe APIs')}."
    languages:
      - python
    severity: ERROR
    metadata:
...
def handle_synthesize_secure_patch(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    remediation_notes = data.get("patch_strategy", "Apply defensive validation and input sanitization")
    patched_code = code

    # Basic automated heuristic replacements for common CWEs
    if cwe_id == "CWE-502" and "pickle." in code:
        patched_code = code.replace("import pickle", "import json").replace("pickle.loads(", "json.loads(").replace("pickle.load(", "json.load(")
        ...
        patched_code = "# TODO: Ensure parameterized tuples: cursor.execute(query, (params,))\n" + code
...
def handle_check_threat_coverage(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "rating": "A+ (Excellent)" if coverage_score >= 0.8 else "B (Moderate)" if coverage_score >= 0.5 else "C (Needs Attention)",
        "breakdown": breakdown,
    }
